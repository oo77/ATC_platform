  ...
  >
    <div class="space-y-6">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">
        Выберите язык, на котором хотите проходить тест. После выбора язык
        изменить будет невозможно.
      </p>
      ...
      <div v-else-if="error" class="py-6 text-center">
        <div
          class="w-12 h-12 rounded-full bg-danger/10 flex items-center justify-center mx-auto mb-3"
        >
          <svg
      ...
      <!-- Предупреждение -->
      <div
        class="flex items-start gap-3 p-3 rounded-lg bg-warning/10 border border-warning/20"
      >
        <svg
    ...
    <template #footer>
      <div class="flex gap-3 justify-end">
        <UiButton variant="outline" @click="handleClose" :disabled="confirming">
          Отмена
        </UiButton>
        <UiButton
  ...
  try {
    ...
    if (response.success) {
      ...
      if (languages.value.length === 0) {
        error.value =
          "Для этого теста нет доступных языков. Обратитесь к администратору.";
      }
    } else {

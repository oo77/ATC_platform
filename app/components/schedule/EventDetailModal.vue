<template>
  ...
  >
    ...
    <div v-else-if="event" class="space-y-6">
      ...
      <!-- Детали занятия -->
      <div
        class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-meta-4 rounded-lg"
      >
        <!-- Дата и время -->
        <div class="flex items-center gap-3">
          <div
        ...
        <div v-if="event.group" class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-lg bg-success/10 dark:bg-success/20 flex items-center justify-center"
          >
            <svg
          ...
          <div>
            ...
            <NuxtLink
              v-if="event.groupId || event.group.id"
              :to="`/groups/${event.groupId || event.group.id}`"
              class="font-medium text-primary hover:underline inline-flex items-center gap-1 group/link"
              @click="handleClose"
              title="Перейти к странице группы"
            >
              <span>{{ event.group.code }}</span>
              <svg
                class="w-3.5 h-3.5 opacity-70 group-hover/link:opacity-100 transition-opacity"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p v-else class="font-medium text-black dark:text-white">
              {{ event.group.code }}
            </p>
          </div>
        </div>
        ...
        <div v-if="event.instructor" class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-lg bg-warning/10 dark:bg-warning/20 flex items-center justify-center"
          >
            <svg
        ...
        <div v-if="event.classroom" class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-lg bg-danger/10 dark:bg-danger/20 flex items-center justify-center"
          >
            <svg
      ...
      <div v-if="event.group" class="space-y-3">
        <div class="flex items-center justify-between">
          ...
          <span
            class="px-3 py-1 bg-primary/10 dark:bg-primary/20 text-primary rounded-full text-sm font-medium"
          >
        ...
        >
          <div class="flex items-center gap-2 text-gray-500">
            <div
              class="inline-block h-5 w-5 animate-spin rounded-full border-2 border-solid border-primary border-r-transparent"
            ></div>
        ...
        >
          <div class="max-h-64 overflow-y-auto">
            <table class="w-full">
              ...
              <tbody
                class="bg-white dark:bg-boxdark divide-y divide-stroke dark:divide-strokedark"
              >
                <tr
                ...
                >
                  ...
                  <td class="px-4 py-3 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                      <div
                        class="w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center text-primary text-xs font-medium"
                      >

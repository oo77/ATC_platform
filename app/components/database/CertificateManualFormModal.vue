<template>
  <UiModal
    ...
    :title="
      preselectedStudent
        ? `Добавить сертификат: ${preselectedStudent.fullName}`
  ...
  >
    <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          >
            ...
            <div class="p-4 space-y-4">
              ...
              <div v-if="studentMode === 'existing'" class="space-y-3">
                ...
                <p v-if="errors.studentId" class="text-sm text-danger">
                  {{ errors.studentId }}
                </p>
              </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      :class="{
                        'border-danger': errors['studentData.fullName'],
                      }"
                    />
                    <p
                      v-if="errors['studentData.fullName']"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      class="w-full rounded border-[1.5px] border-stroke bg-transparent px-4 py-2.5 font-medium outline-none transition focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                      :class="{
                        'border-danger': errors['studentData.organization'],
                      }"
                    />
                    <p
                      v-if="errors['studentData.organization']"
                      class="mt-1 text-sm text-danger"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      class="w-full rounded border-[1.5px] border-stroke bg-transparent px-4 py-2.5 font-medium outline-none transition focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                      :class="{
                        'border-danger': errors['studentData.position'],
                      }"
                    />
                    <p
                      v-if="errors['studentData.position']"
                      class="mt-1 text-sm text-danger"
                    >
              ...
              <div v-if="courseMode === 'existing'">
                ...
                >
                  ...
                  <option
                    ...
                    :value="course.id"
                  >
                    {{ course.name }} ({{ course.code || "без кода" }},
                    {{ course.hours || "?" }} ч.)
                  </option>
                </select>
            ...
            <div class="p-4 space-y-4">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  />
                  <p v-if="errors.expiryDate" class="mt-1 text-sm text-danger">
                    {{ errors.expiryDate }}
                  </p>
      ...
      <!-- Кнопки -->
      <div
        class="flex justify-end gap-4 pt-4 border-t border-stroke dark:border-strokedark"
      >
        <UiButton
          type="button"
          variant="secondary"
...
async function handleSubmit() {
  ...
  try {
    const payload = {
      // Слушатель
      studentId: props.preselectedStudent
        ? props.preselectedStudent.id
        : studentMode.value === "existing"
          ? selectedStudent.value?.id
          : undefined,
      createNewStudent:
        !props.preselectedStudent && studentMode.value === "new",
      studentData:
        !props.preselectedStudent && studentMode.value === "new"
          ? {
      ...
      // Файл
      fileUrl:
        fileInputMode.value === "url" && formData.value.fileUrl
          ? formData.value.fileUrl.trim()
          : undefined,
    };

import { U as UserRole, h as createError, f as executeQuery, x as getHeader, y as verifyToken } from './nitro.mjs';
...
var Permission = /* @__PURE__ */ ((Permission2) => {
  ...
  Permission2["LIBRARY_READ"] = "library:read";
  Permission2["LIBRARY_UPLOAD"] = "library:upload";
  Permission2["LIBRARY_MANAGE"] = "library:manage";
  Permission2["LIBRARY_ACCESS_MANAGE"] = "library:access_manage";
  return Permission2;
})(Permission || {});
const ROLE_PERMISSIONS = {
  ...
  [UserRole.MANAGER]: [
    ...
    "logs:view" /* LOGS_VIEW */,
    "library:view" /* LIBRARY_VIEW */,
    "library:read" /* LIBRARY_READ */,
    "library:upload" /* LIBRARY_UPLOAD */,
    "library:manage" /* LIBRARY_MANAGE */,
    "library:access_manage" /* LIBRARY_ACCESS_MANAGE */
  ],
  [UserRole.TEACHER]: [
    "dashboard:view" /* DASHBOARD_VIEW */,
    "students:view" /* STUDENTS_VIEW */,
...
async function getInstructorByUserId(userId) {
  try {
    console.log(
      `[Permissions] getInstructorByUserId called with userId: ${userId}`
    );
    ...
    if (rows && rows.length > 0) {
      console.log(
        `[Permissions] Found instructor: ${rows[0].fullName} (ID: ${rows[0].id})`
      );
      return rows[0];
    } else {
...
async function getTeacherGroups(instructorId) {
  try {
    ...
    console.log(
      `[Permissions] getTeacherGroups for ${instructorId}: found ${groupIds.length} groups`
    );
    return groupIds;
  } catch (error) {
...
async function requirePermission(event, permission) {
  ...
  if (!roleHasPermission(context.role, permission)) {
    console.warn(
      `[Permissions] Access denied: user ${context.userId} (${context.role}) lacks permission ${permission}`
    );
    throw createError({
      statusCode: 403,
...
async function requireAnyPermission(event, permissions) {
  ...
  if (!roleHasAnyPermission(context.role, permissions)) {
    console.warn(
      `[Permissions] Access denied: user ${context.userId} (${context.role}) lacks any of [${permissions.join(", ")}]`
    );
    throw createError({
      statusCode: 403,
      statusMessage: "Forbidden",
...
async function requireAllPermissions(event, permissions) {
  ...
  if (!roleHasAllPermissions(context.role, permissions)) {
    const missing = permissions.filter(
      (p) => !roleHasPermission(context.role, p)
    );
    console.warn(
      `[Permissions] Access denied: user ${context.userId} (${context.role}) lacks permissions [${missing.join(", ")}]`
    );
    throw createError({
      statusCode: 403,
      statusMessage: "Forbidden",
...
export { Permission as P, requireAnyPermission as a, getPermissionContext as b, roleHasPermission as c, getTeacherGroups as d, requireAllPermissions as e, getStudentGroups as f, getStudentByUserId as g, requirePermission as r };

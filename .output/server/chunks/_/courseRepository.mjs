...
async function getCoursesPaginated(params = {}) {
  ...
  const templateIds = [
    ...new Set(
      rows.filter((r) => r.certificate_template_id).map((r) => r.certificate_template_id)
    )
  ];
...
async function createCourse(data) {
  ...
  const totalHours = data.disciplines?.reduce(
...
async function updateCourse(id, data) {
  ...
  if (data.certificateValidityMonths !== void 0) {

...
async function generateCertificateNumber(templateId, courseCode) {
  return executeTransaction(async (connection) => {
    ...
    while (attempts < maxAttempts) {
      ...
      certificateNumber = format.replace("{YY}", year).replace("{YYYY}", fullYear).replace("{CODE}", courseCode).replace("{NUM}", paddedNum);
    ...
    await connection.query(
      ...
      [attemptNumber, /* @__PURE__ */ new Date(), templateId]
    );
    console.log(
...
async function syncCertificateNumberCounter(templateId) {
  return executeTransaction(async (connection) => {
    const [template] = await connection.query(
    ...
    const [maxCerts] = await connection.query(
    ...
    if (synced) {
      await connection.query(
        "UPDATE certificate_templates SET last_number = ?, updated_at = ? WHERE id = ?",
        [newCounter, /* @__PURE__ */ new Date(), templateId]
      );
      console.log(
...
async function checkStudentEligibility(studentId, groupId) {
  ...
  const debugEvents = await executeQuery(
    `SELECT 
       se.id,
       se.title,
  ...
  console.log(
    `  Regular events: ${debugEvents.filter((e) => e.event_type === "regular").length}`
  );
  console.log(
    `  Retakes for this student: ${debugEvents.filter((e) => e.event_type === "retake_for_student").length}`
  );
  console.log(
    `  Retakes for others: ${debugEvents.filter((e) => e.event_type === "retake_for_others").length}`
  );
  console.log(
  ...
  attendanceDetails.forEach((rec) => {
    console.log(
      `    - ${rec.event_title}: ${rec.hours_attended}/${rec.max_hours}h (allowed_ids: ${rec.allowed_student_ids || "NULL"})`
    );
  });
  ...
  const totalAttendancePercent = totalHours > 0 ? Math.min(100, totalAttendedHours / totalHours * 100) : 0;
  ...
  console.log(
    `[Eligibility] Student ${studentId}: attendance=${totalAttendancePercent.toFixed(
      1
    )}% (${totalAttendedHours}/${totalHours}h), grades=${gradesCount}/${totalDisciplines}, avgGrade=${averageGrade?.toFixed(1) || "N/A"}`
  );
  return {
    studentId,

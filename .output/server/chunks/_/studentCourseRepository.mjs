...
async function getStudentCourseDetails(userId, groupId) {
  ...
  const courseQuery = `
    ...
    LIMIT 1
  `;
  const courseRows = await executeQuery(courseQuery, [
    studentId,
    groupId
  ]);
  ...
  `;
  const attRows = await executeQuery(attendanceCountQuery, [
    studentId,
    groupId
  ]);
  ...
  `;
  try {
    const scheduleRows = await executeQuery(scheduleQuery, [
      studentId,
      studentId,
      groupId
    ]);
    ...
    const scheduleQuerySimple = `
        ...
        ORDER BY se.start_time ASC
      `;
    const scheduleRows = await executeQuery(scheduleQuerySimple, [
      studentId,
      groupId
    ]);
    const schedule = scheduleRows.map((row) => ({
...
async function getStudentDashboardStats(userId) {
  ...
  const upcomingEventsQuery = `
    ...
    LIMIT 3
  `;
  const upcomingEvents = await executeQuery(upcomingEventsQuery, [
    studentId,
    now
  ]);
  ...
  `;
  const activeCourses = await executeQuery(activeCoursesQuery, [
    now,
    studentId
  ]);
  ...
  try {
    const deadlinesQuery = `
      ...
      LIMIT 3
    `;
    upcomingDeadlines = await executeQuery(deadlinesQuery, [
      studentId,
      studentId,
      now,
      now
...
export { getStudentCourses as a, getStudentCourseDetails as b, getStudentDashboardStats as g };

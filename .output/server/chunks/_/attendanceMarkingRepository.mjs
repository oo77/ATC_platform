...
async function checkMarkingAccess(scheduleEventId, userId, userRole, instructorId) {
  ...
  if (now <= deadline) {
    ...
    console.log(
      `[checkMarkingAccess] Deadline passed. Checking role: userRole="${userRole}", isAdmin=${userRole === "ADMIN"}, isManager=${userRole === "MANAGER"}`
    );
    if (userRole === "ADMIN" || userRole === "MANAGER") {
      ...
      return {
        ...
        // Флаг для фронтенда: не требовать подтверждения
      };
    }

...
function resolveVariable(key, context) {
  ...
  switch (key) {
    ...
    case "instructor.shortName":
      return context.instructor?.fullName ? getShortName(context.instructor.fullName) : "";
...
async function generateCertificateHtml(templateData, context) {
  ...
  const fontLinks = Array.from(fonts).filter((f) => !["Arial", "Times New Roman", "Georgia"].includes(f)).map(
    (f) => `<link href="https://fonts.googleapis.com/css2?family=${encodeURIComponent(f)}:wght@400;700&display=swap" rel="stylesheet">`
  ).join("\n");
  return `
    <!DOCTYPE html>
    <html>

...
import { defineComponent, ref, computed, watch, mergeProps, withCtx, createBlock, createTextVNode, openBlock, createVNode, unref, toDisplayString, reactive, withModifiers, withDirectives, vModelText, vModelCheckbox, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderClass, ssrRenderStyle, ssrRenderAttr, ssrRenderTeleport, ssrIncludeBooleanAttr, ssrLooseContain } from 'vue/server-renderer';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        const response = await authFetch(`/api/certificates/templates/${props.template.id}`, {
          method: "PUT",
          body: {
            name: form.name.trim(),
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const loadPreview = async () => {
      ...
      try {
        const response = await authFetch(`/api/certificates/templates/${id.value}/preview`);
    ...
    const duplicateTemplate = async () => {
      ...
      try {
        const response = await authFetch(`/api/certificates/templates/${id.value}/duplicate`, {
          method: "POST"
        });
        if (response.success) {
    ...
    const syncCounter = async () => {
      ...
      try {
        const response = await authFetch(`/api/certificates/templates/${id.value}/sync-counter`, {
          method: "POST"
        });
        if (response.success) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        if (unref(previewLoading)) {
          _push(`<div class="absolute inset-0 flex flex-col items-center justify-center bg-white/80 dark:bg-gray-900/80 z-10"><div class="h-10 w-10 animate-spin rounded-full border-4 border-solid border-primary border-t-transparent"></div><p class="mt-2 text-sm text-gray-500"> Загрузка предпросмотра... </p></div>`);
        } else {
          _push(`<!---->`);
        }
        if (!unref(template).templateData) {
          ...
          _push(`</div>`);
        } else if (unref(previewHtml)) {
          _push(`<div class="w-full h-full flex items-center justify-center p-4"><div class="bg-white shadow-lg relative overflow-hidden transition-all duration-300" style="${ssrRenderStyle({
            width: `${(unref(previewData)?.width || 0) * unref(previewScale)}px`,
            height: `${(unref(previewData)?.height || 0) * unref(previewScale)}px`
          })}"><div class="origin-top-left" style="${ssrRenderStyle({
            width: `${unref(previewData)?.width || 1123}px`,
            height: `${unref(previewData)?.height || 794}px`,
            transform: `scale(${unref(previewScale)})`
          })}"><iframe${ssrRenderAttr("srcdoc", unref(previewHtml))} class="w-full h-full border-0 block" sandbox="allow-same-origin" scrolling="no"></iframe></div></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              return [
                ...
                createTextVNode(" Синхронизировать счетчик ")
              ];
            }
          }),
          _: 1
        }, _parent));
        _push(`<p class="text-xs text-gray-500 dark:text-gray-400"> Используйте, если счетчик сбился после импорта или возникли дубликаты номеров </p></div></div><div class="rounded-xl bg-white dark:bg-boxdark shadow-md p-6"><h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4"> Действия </h3><div class="space-y-3">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: `/certificates/templates/${unref(template).id}/editor`,
          class: "block"
        }, {
        ...
        _push(ssrRenderComponent(_component_UiButton, {
          ...
          loading: unref(isDuplicating)
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"${_scopeId}></path></svg> Дублировать шаблон `);
            } else {
              return [
                (openBlock(), createBlock("svg", {
                  class: "w-4 h-4 mr-2",
                ...
                createTextVNode(" Дублировать шаблон ")
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=index-BaT1qu7c.mjs.map

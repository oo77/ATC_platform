...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10" }, _attrs))}><div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-title-md2 font-bold text-black dark:text-white"> Импорт сертификатов </h2><p class="mt-1 text-sm text-gray-600 dark:text-gray-400"> Массовый импорт сертификатов из Excel файла </p></div><div class="flex gap-2"><button class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg> AI Импорт сертификатов </button>`);
      ...
      _push(`</div></div><div class="mb-8"><div class="flex items-center justify-between"><!--[-->`);
      ssrRenderList(steps, (step, index) => {
        ...
        _push(`</div><p class="${ssrRenderClass([{
          "text-primary font-semibold": currentStep.value === index + 1
        }, "mt-2 text-xs sm:text-sm font-medium text-center text-gray-700 dark:text-gray-300"])}">${ssrInterpolate(step.label)}</p></div>`);
...
//# sourceMappingURL=import-certificates-BW1ZYB16.mjs.map

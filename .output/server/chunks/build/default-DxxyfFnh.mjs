...
import { k as useState, P as Permission, e as useAuth } from './server.mjs';
...
import { u as useImportStore } from './useImportStore-CLH1yiE_.mjs';
...
import { G as GridIcon, C as CalenderIcon, A as AcademicCapIcon, U as UserGroupIcon, a as ClipboardCheckIcon, b as CertificateIcon, c as UserCircleIcon, S as SettingsIcon } from './GridIcon-mmTMaVf3.mjs';
...
import { D as DocsIcon } from './DocsIcon-DpYpw5J-.mjs';
import '../_/nitro.mjs';
import 'grammy';
import 'uuid';
import 'node:http';
...
const _sfc_main$c = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getUserLink = (user2) => {
      if (user2.role === "STUDENT" && user2.studentId) {
        return `/students/${user2.studentId}`;
      }
      if (user2.role === "TEACHER" && user2.instructorId) {
        return `/instructors/${user2.instructorId}`;
      }
      return `/users/${user2.id}`;
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (canSearch.value) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "hidden lg:block" }, _attrs))}><form><div class="relative"><button class="absolute -translate-y-1/2 left-4 top-1/2 pointer-events-none" type="button"><svg class="fill-gray-500 dark:fill-gray-400" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M3.04175 9.37363C3.04175 5.87693 5.87711 3.04199 9.37508 3.04199C12.8731 3.04199 15.7084 5.87693 15.7084 9.37363C15.7084 12.8703 12.8731 15.7053 9.37508 15.7053C5.87711 15.7053 3.04175 12.8703 3.04175 9.37363ZM9.37508 1.54199C5.04902 1.54199 1.54175 5.04817 1.54175 9.37363C1.54175 13.6991 5.04902 17.2053 9.37508 17.2053C11.2674 17.2053 13.003 16.5344 14.357 15.4176L17.177 18.238C17.4699 18.5309 17.9448 18.5309 18.2377 18.238C18.5306 17.9451 18.5306 17.4703 18.2377 17.1774L15.418 14.3573C16.5365 13.0033 17.2084 11.2669 17.2084 9.37363C17.2084 5.04817 13.7011 1.54199 9.37508 1.54199Z" fill=""></path></svg></button><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Поиск пользователей по ФИО или ПИНФЛ..." class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-white/3 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 xl:w-[430px]">`);
        if (searchQuery.value) {
          _push(`<button type="button" class="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>`);
        } else {
          _push(`<!---->`);
        }
        if (showResults.value && (searchResults.value.length > 0 || isSearching.value)) {
          _push(`<div class="absolute z-50 mt-2 w-full rounded-lg border border-stroke bg-white dark:bg-boxdark dark:border-strokedark shadow-lg max-h-80 overflow-auto">`);
          if (isSearching.value) {
            _push(`<div class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 text-center"><div class="flex items-center justify-center gap-2"><svg class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>Поиск...</span></div></div>`);
          } else {
            _push(`<div class="py-2"><!--[-->`);
            ssrRenderList(searchResults.value, (user2) => {
              _push(ssrRenderComponent(_component_NuxtLink, {
                key: user2.id,
                to: getUserLink(user2),
                ...
                default: withCtx((_, _push2, _parent2, _scopeId) => {
                  if (_push2) {
                    _push2(`<div class="shrink-0 w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold"${_scopeId}>${ssrInterpolate(getUserInitials(user2.name))}</div><div class="flex-1 min-w-0"${_scopeId}><div class="flex items-center gap-2"${_scopeId}><p class="text-sm font-medium text-gray-900 dark:text-white truncate"${_scopeId}>${ssrInterpolate(user2.name)}</p><span class="${ssrRenderClass([getRoleBadgeClass(user2.role), "inline-flex items-center px-2 py-0.5 rounded text-xs font-medium"])}"${_scopeId}>${ssrInterpolate(getRoleLabel(user2.role))}</span></div><div class="flex items-center gap-3 mt-1"${_scopeId}>`);
                    if (user2.email) {
                      _push2(`<p class="text-xs text-gray-500 dark:text-gray-400 truncate"${_scopeId}>${ssrInterpolate(user2.email)}</p>`);
                    } else {
                      _push2(`<!---->`);
                    }
                    if (user2.pinfl) {
                      _push2(`<p class="text-xs text-gray-500 dark:text-gray-400"${_scopeId}> ПИНФЛ: ${ssrInterpolate(user2.pinfl)}</p>`);
                    } else {
                      _push2(`<!---->`);
                    }
                    _push2(`</div></div><svg class="w-5 h-5 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"${_scopeId}></path></svg>`);
                  } else {
                    return [
                      createVNode("div", { class: "shrink-0 w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold" }, toDisplayString(getUserInitials(user2.name)), 1),
                      createVNode("div", { class: "flex-1 min-w-0" }, [
                        createVNode("div", { class: "flex items-center gap-2" }, [
                          createVNode("p", { class: "text-sm font-medium text-gray-900 dark:text-white truncate" }, toDisplayString(user2.name), 1),
                          createVNode("span", {
                            class: ["inline-flex items-center px-2 py-0.5 rounded text-xs font-medium", getRoleBadgeClass(user2.role)]
                          }, toDisplayString(getRoleLabel(user2.role)), 3)
                        ]),
                        createVNode("div", { class: "flex items-center gap-3 mt-1" }, [
                          user2.email ? (openBlock(), createBlock("p", {
                            key: 0,
                            class: "text-xs text-gray-500 dark:text-gray-400 truncate"
                          }, toDisplayString(user2.email), 1)) : createCommentVNode("", true),
                          user2.pinfl ? (openBlock(), createBlock("p", {
                            key: 1,
                            class: "text-xs text-gray-500 dark:text-gray-400"
                          }, " ПИНФЛ: " + toDisplayString(user2.pinfl), 1)) : createCommentVNode("", true)
                        ])
                      ]),
        ...
        if (showResults.value && !isSearching.value && searchResults.value.length === 0 && searchQuery.value.length >= 2) {
          _push(`<div class="absolute z-50 mt-2 w-full rounded-lg border border-stroke bg-white dark:bg-boxdark dark:border-strokedark shadow-lg"><div class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 text-center"> Пользователи не найдены </div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></form></div>`);
...
const SearchBar = Object.assign(_sfc_main$c, { __name: "LayoutHeaderSearchBar" });
...
const DatabaseIcon = Object.assign(_sfc_main$5, { __name: "IconsDatabaseIcon" });
const _sfc_main$4 = {};
...
const FolderIcon = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main$4, [["ssrRender", _sfc_ssrRender$1]]), { __name: "IconsFolderIcon" });
const _sfc_main$3 = {};
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  _push(`<svg${ssrRenderAttrs(mergeProps({
    ...
    "stroke-width": "1.5",
    stroke: "currentColor"
  }, _attrs))}><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25"></path></svg>`);
}
const _sfc_setup$3 = _sfc_main$3.setup;
_sfc_main$3.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/icons/BookIcon.vue");
  return _sfc_setup$3 ? _sfc_setup$3(props, ctx) : void 0;
};
const BookIcon = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main$3, [["ssrRender", _sfc_ssrRender]]), { __name: "IconsBookIcon" });
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
        ...
        const filteredItems = group.items.map((item) => {
          ...
          if (item.subItems) {
            const filteredSubItems = item.subItems.filter((subItem) => {
              ...
              if (subItem.anyPermissions && !hasAnyPermission(subItem.anyPermissions)) {
                return false;
              }
              return true;
            });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))} data-v-7c52ce76><div class="${ssrRenderClass([
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex items-center justify-center h-full" data-v-7c52ce76${_scopeId}>`);
            if (unref(isExpanded) || unref(isHovered) || unref(isMobileOpen)) {
              _push2(`<img${ssrRenderAttr("src", _imports_0$1)} alt="Logo" class="h-24 object-contain mx-auto transition-all duration-300 group-hover:scale-105" data-v-7c52ce76${_scopeId}>`);
            } else {
              _push2(`<img${ssrRenderAttr("src", _imports_1)} alt="Logo" class="h-10 w-10 object-contain mx-auto rounded-xl shadow-md transition-all duration-300 hover:rotate-6" data-v-7c52ce76${_scopeId}>`);
            }
            _push2(`</div>`);
          } else {
            return [
      ...
      _push(`</div><div class="flex flex-col flex-1 overflow-y-auto overflow-x-hidden no-scrollbar py-4 px-3 space-y-6" data-v-7c52ce76><!--[-->`);
      ssrRenderList(menuGroups.value, (menuGroup, groupIndex) => {
        _push(`<nav data-v-7c52ce76>`);
        if (menuGroup.title) {
          ...
          ])}" data-v-7c52ce76>${ssrInterpolate(menuGroup.title)}</h3>`);
        } else {
          _push(`<!---->`);
        }
        if (!unref(isExpanded) && !unref(isHovered) && menuGroup.title && groupIndex > 0) {
          _push(`<div class="h-px bg-gray-100 dark:bg-gray-800 mx-4 mb-4 mt-2" data-v-7c52ce76></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<ul class="space-y-1.5" data-v-7c52ce76><!--[-->`);
        ssrRenderList(menuGroup.items, (item, index) => {
          _push(`<li data-v-7c52ce76>`);
          if (item.subItems && item.subItems.length > 0) {
            _push(`<div data-v-7c52ce76><button class="${ssrRenderClass([
            ...
            ])}" data-v-7c52ce76><div class="flex items-center gap-3 min-w-0" data-v-7c52ce76><div class="${ssrRenderClass([
            ...
            if (isSubmenuOpen(groupIndex, index)) {
              _push(`<span class="absolute inset-0 bg-blue-500/10 dark:bg-blue-400/10 rounded-lg blur-sm scale-150 opacity-50" data-v-7c52ce76></span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div><span class="truncate font-medium text-sm transition-all duration-300" style="${ssrRenderStyle(unref(isExpanded) || unref(isHovered) || unref(isMobileOpen) ? null : { display: "none" })}" data-v-7c52ce76>${ssrInterpolate(item.name)}</span></div>`);
            _push(ssrRenderComponent(ChevronDownIcon, {
            ...
            _push(`</button><div class="overflow-hidden" style="${ssrRenderStyle(isSubmenuOpen(groupIndex, index) && (unref(isExpanded) || unref(isHovered) || unref(isMobileOpen)) ? null : { display: "none" })}" data-v-7c52ce76><ul class="pt-1 pb-2 pl-[1.15rem] ml-3 border-l-[1.5px] border-gray-100 dark:border-gray-800 space-y-1" data-v-7c52ce76><!--[-->`);
                ...
                default: withCtx((_, _push2, _parent2, _scopeId) => {
                  if (_push2) {
                    if (isActive(subItem.path, subItem.excludePaths)) {
                      _push2(`<span class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-3 bg-blue-500 rounded-r-full" data-v-7c52ce76${_scopeId}></span>`);
                    } else {
                      _push2(`<!---->`);
                    }
              ...
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  if (!isActive(item.path ?? "", item.excludePaths)) {
                    _push2(`<div class="absolute inset-0 bg-linear-to-r from-blue-500/0 via-blue-500/5 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" data-v-7c52ce76${_scopeId}></div>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  ...
                  ])}" data-v-7c52ce76${_scopeId}>`);
                  ssrRenderVNode(_push2, createVNode(resolveDynamicComponent(item.icon), { class: "w-5 h-5" }, null), _parent2, _scopeId);
                  _push2(`</div><span class="ml-3 truncate font-medium text-sm transition-opacity duration-300 relative z-10" style="${ssrRenderStyle(unref(isExpanded) || unref(isHovered) || unref(isMobileOpen) ? null : { display: "none" })}" data-v-7c52ce76${_scopeId}>${ssrInterpolate(item.name)}</span>`);
                  if (!unref(isExpanded) && !unref(isHovered) && !unref(isMobileOpen)) {
                    _push2(`<div class="absolute left-[calc(100%+10px)] top-1/2 -translate-y-1/2 px-3 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 z-60 shadow-xl whitespace-nowrap translate-x--2 group-hover:translate-x-0" data-v-7c52ce76${_scopeId}>${ssrInterpolate(item.name)} <div class="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-[4px] w-2 h-2 bg-gray-900 rotate-45" data-v-7c52ce76${_scopeId}></div></div>`);
                  } else {
                    _push2(`<!---->`);
                  }
                } else {
      ...
      if (unref(isExpanded) || unref(isHovered) || unref(isMobileOpen)) {
        ...
        ])}" data-v-7c52ce76><div class="flex flex-col items-center justify-center space-y-2 text-center" data-v-7c52ce76><div class="flex items-center gap-2" data-v-7c52ce76><span class="text-xs font-semibold text-gray-900 dark:text-white" data-v-7c52ce76>АТЦ Платформа</span><span class="px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-100 dark:border-blue-800" data-v-7c52ce76> v1.0 </span></div><p class="text-[10px] text-gray-400 dark:text-gray-500 leading-relaxed" data-v-7c52ce76> © ${ssrInterpolate((/* @__PURE__ */ new Date()).getFullYear())} АТЦ Платформа.<br data-v-7c52ce76> Все права защищены. </p></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</aside>`);
...
const AppSidebar = /* @__PURE__ */ Object.assign(_export_sfc(_sfc_main$2, [["__scopeId", "data-v-7c52ce76"]]), { __name: "LayoutAppSidebar" });
...
//# sourceMappingURL=default-DxxyfFnh.mjs.map

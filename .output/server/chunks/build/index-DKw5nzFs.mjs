...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const loadTemplates = async () => {
      loading.value = true;
      try {
        const response = await authFetch("/api/certificates/templates");
        if (response.success) {
    ...
    const getPreviewBackgroundStyle = (template) => {
    ...
    const getTextStyle = (element) => {
      return {
        ...
        height: "100%",
        overflow: "hidden",
        // Обрезаем по высоте контейнера
        display: "flex",
        flexDirection: "column",
        justifyContent: "center"
        // Вертикальное центрирование (как в редакторе)
      };
    };
    return (_ctx, _push, _parent, _attrs) => {
      const _component_UiButton = __nuxt_component_1;
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(templates), (template) => {
          ...
          if (template.templateData) {
            _push(`<div class="absolute inset-0 overflow-hidden bg-gray-100 flex items-center justify-center"><div class="relative bg-white shadow-lg overflow-hidden shrink-0" style="${ssrRenderStyle({
              width: `${template.templateData.width}px`,
              height: `${template.templateData.height}px`,
              transform: "scale(0.2)",
              // Масштаб ~20%
              transformOrigin: "center center"
            })}"><div class="absolute inset-0 z-0" style="${ssrRenderStyle(getPreviewBackgroundStyle(template))}"></div><!--[-->`);
            ssrRenderList(template.templateData.elements, (element) => {
              _push(`<div style="${ssrRenderStyle(getElementStyle(element))}" class="pointer-events-none">`);
              if (element.type === "text") {
                _push(`<div style="${ssrRenderStyle(getTextStyle(element))}">${ssrInterpolate(element.content)}</div>`);
              } else if (element.type === "variable") {
                _push(`<div style="${ssrRenderStyle(getTextStyle(element))}" class="text-primary font-bold opacity-70"> [${ssrInterpolate(element.variableKey)}] </div>`);
              } else if (element.type === "image") {
                _push(`<img${ssrRenderAttr("src", element.src)} class="w-full h-full object-contain">`);
              } else if (element.type === "qr") {
                _push(`<div class="w-full h-full bg-black/10 flex items-center justify-center"><svg class="w-1/2 h-1/2 text-gray-400" fill="currentColor" viewBox="0 0 24 24"><path d="M3 3h6v6H3V3zm2 2v2h2V5H5zm8-2h6v6h-6V3zm2 2v2h2V5h-2zM3 11h6v6H3v-6zm2 2v2h2v-2H5zm13-2h-3v3h3v-3zm0 3h-3v3h3v-3z"></path></svg></div>`);
            ...
            _push(`<!--]--></div><div class="absolute inset-0 bg-linear-to-t from-black/10 to-transparent pointer-events-none"></div></div>`);
          ...
          if (template.originalFileUrl || template.backgroundUrl) {
            _push(`<div class="${ssrRenderClass([
              template.variables && template.variables.length > 0 ? "bg-success/80 text-white" : "bg-warning/80 text-white",
              "absolute bottom-3 right-3 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 backdrop-blur-sm"
            ])}">`);
          ...
          _push(`<button class="p-2 bg-white rounded-lg hover:bg-green-50 transition-colors" title="Дублировать"><svg class="w-5 h-5 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>`);
...
//# sourceMappingURL=index-DKw5nzFs.mjs.map

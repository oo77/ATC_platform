...
import { defineComponent, computed, ref, watch, mergeProps, unref, withCtx, createTextVNode, toDisplayString, createBlock, openBlock, createVNode, createCommentVNode, withDirectives, isRef, vModelText, Fragment, renderList, vModelRadio, vModelSelect, resolveDynamicComponent, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrInterpolate, ssrRenderComponent, ssrRenderClass, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderAttr, ssrRenderTeleport, ssrRenderVNode } from 'vue/server-renderer';
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  props: {
    ...
    column: {},
    studentId: {},
    isRetakeTarget: { type: Boolean },
    replacementGrade: {},
    readOnly: { type: Boolean }
  },
  emits: ["update", "requireApproval"],
...
${a.notes}`;
    ...
    const gradeTooltip = computed(() => {
      ...
      if (props.isRetakeTarget && props.replacementGrade !== null && props.replacementGrade !== void 0) {
        text += `
🔄 Пересдано. Новая оценка: ${props.replacementGrade}`;
      }
...
${g.notes}`;
    ...
    const saveAttendance = async () => {
      ...
      try {
        ...
        if (response.requiresApproval) {
          ...
          toast.warning(
            response.message || "Срок отметки истёк. Требуется одобрение администратора"
          );
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.cell.isHidden) {
        _push(`<div class="w-10 h-8 flex items-center justify-center text-gray-300 dark:text-gray-600" title="Не участвует в этом занятии"> — </div>`);
      } else if (__props.column.hasGrade) {
        _push(`<div class="flex flex-col items-center gap-1"><button class="${ssrRenderClass([[unref(attendanceButtonClass), { "opacity-70 cursor-not-allowed": __props.readOnly }], "w-10 h-6 rounded text-xs font-medium transition-all"])}"${ssrIncludeBooleanAttr(__props.readOnly) ? " disabled" : ""}${ssrRenderAttr("title", __props.readOnly ? "Редактирование недоступно (Архив или нет прав)" : unref(attendanceTooltip))}>${ssrInterpolate(unref(attendanceDisplay))}</button><button class="${ssrRenderClass([[
          unref(gradeButtonClass),
          {
            "line-through opacity-70": __props.isRetakeTarget && __props.replacementGrade !== null,
            "opacity-70 cursor-not-allowed": __props.readOnly
          }
        ], "w-10 h-6 rounded text-xs font-medium transition-all relative"])}"${ssrIncludeBooleanAttr(__props.readOnly) ? " disabled" : ""}${ssrRenderAttr("title", __props.readOnly ? "Редактирование недоступно (Архив или нет прав)" : unref(gradeTooltip))}>${ssrInterpolate(unref(gradeDisplay))} `);
        if (__props.isRetakeTarget && __props.replacementGrade !== null) {
          _push(`<span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-red-500" title="Есть пересдача"></span>`);
        } else if (props.cell.grade?.isFromTest && !props.cell.grade?.isModified) {
          _push(`<span class="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-blue-500" title="Автоматическая оценка из теста"></span>`);
        } else if (props.cell.grade?.isModified) {
        ...
        _push(`<button class="${ssrRenderClass([[unref(attendanceButtonClass), { "opacity-70 cursor-not-allowed": __props.readOnly }], "w-10 h-8 rounded text-xs font-medium transition-all"])}"${ssrIncludeBooleanAttr(__props.readOnly) ? " disabled" : ""}${ssrRenderAttr("title", __props.readOnly ? "Редактирование недоступно (Архив или нет прав)" : unref(attendanceTooltip))}>${ssrInterpolate(unref(attendanceDisplay))}</button>`);
      }
          ...
          if (_push2) {
            ...
            ssrRenderList(unref(quickOptions), (option) => {
              _push2(`<button class="${ssrRenderClass([
                unref(attendanceInput) === option ? "bg-primary text-white border-primary" : "border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700",
                "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
              ])}"${_scopeId}>${ssrInterpolate(option)}</button>`);
            });
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList([100, 90, 80, 70, 60, 50, 40], (grade) => {
              _push2(`<button class="${ssrRenderClass([
                unref(gradeInput) === grade ? "bg-primary text-white border-primary" : "border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700",
                "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
              ])}"${_scopeId}>${ssrInterpolate(grade)}</button>`);
            });
...
const AttendanceCell = Object.assign(_sfc_main$3, { __name: "AttendanceCell" });
...
${fg.notes}`;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      const _component_UiModal = __nuxt_component_0$1;
      const _component_UiButton = __nuxt_component_1;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "inline-block" }, _attrs))}><button class="${ssrRenderClass([[unref(buttonClass), { "opacity-70 cursor-not-allowed": __props.readOnly }], "px-3 py-1 rounded text-sm font-medium transition-all"])}"${ssrIncludeBooleanAttr(__props.readOnly) ? " disabled" : ""}${ssrRenderAttr("title", __props.readOnly ? "Редактирование недоступно (Архив или нет прав)" : unref(tooltip))}>${ssrInterpolate(unref(displayText))}</button>`);
      _push(ssrRenderComponent(_component_UiModal, {
...
const FinalGradeCell = Object.assign(_sfc_main$2, { __name: "AttendanceFinalGradeCell" });
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  __name: "LateMarkingModal",
  __ssrInlineRender: true,
  props: {
  ...
  emits: ["update:modelValue", "confirm", "request-approval", "cancel"],
  setup(__props, { emit: __emit }) {
    const props = __props;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        if (__props.modelValue) {
          _push2(`<div class="fixed inset-0 z-999 flex items-center justify-center p-4"><div class="absolute inset-0 bg-black/50"></div><div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6 animate-[fadeIn_0.2s_ease-out]"><div class="flex items-start gap-4 mb-4"><div class="${ssrRenderClass([unref(isLateMode) ? "bg-yellow-100 dark:bg-yellow-900/30" : "bg-red-100 dark:bg-red-900/30", "shrink-0 w-12 h-12 rounded-full flex items-center justify-center"])}">`);
          ssrRenderVNode(_push2, createVNode(resolveDynamicComponent(unref(isLateMode) ? unref(AlertTriangle) : unref(Lock)), {
            class: [unref(isLateMode) ? "text-yellow-600" : "text-red-600", "w-6 h-6"]
          }, null), _parent);
          _push2(`</div><div><h3 class="text-lg font-semibold text-gray-900 dark:text-white">${ssrInterpolate(unref(isLateMode) ? "⚠️ Срок отметки истёк" : "🔒 Требуется одобрение")}</h3><p class="text-sm text-gray-500 dark:text-gray-400 mt-1">${ssrInterpolate(unref(isLateMode) ? 'Отметка будет сохранена с пометкой "Опоздание"' : "Для отметки необходимо одобрение администратора")}</p></div></div><div class="${ssrRenderClass([unref(isLateMode) ? "bg-yellow-50 dark:bg-yellow-900/20" : "bg-red-50 dark:bg-red-900/20", "rounded-lg p-4 mb-4"])}"><p class="text-sm"><strong>Занятие:</strong> ${ssrInterpolate(__props.eventTitle)}</p><p class="text-sm mt-1"><strong>Дата:</strong> ${ssrInterpolate(formatDate(__props.eventDate))}</p><p class="text-sm mt-1"><strong>Дедлайн был:</strong> ${ssrInterpolate(formatDate(__props.deadline))}</p></div><div class="mb-4"><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">${ssrInterpolate(unref(requiresApprovalMode) ? "Причина опоздания (обязательно)" : "Причина опоздания (опционально)")} `);
          if (unref(requiresApprovalMode)) {
            _push2(`<span class="text-red-500">*</span>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`</label><textarea rows="3"${ssrRenderAttr("placeholder", unref(requiresApprovalMode) ? "Укажите причину опоздания с отметкой (минимум 10 символов)..." : "Укажите причину, если необходимо...")} class="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-3 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent placeholder-gray-400 dark:placeholder-gray-500">${ssrInterpolate(unref(lateReason))}</textarea>`);
          if (unref(requiresApprovalMode) && unref(lateReason).length > 0 && unref(lateReason).length < 10) {
            _push2(`<p class="text-xs text-red-500 mt-1"> Минимум 10 символов (сейчас: ${ssrInterpolate(unref(lateReason).length)}) </p>`);
          } else {
            _push2(`<!---->`);
          }
          _push2(`</div><div class="flex gap-3 justify-end"><button class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors"> Отмена </button><button${ssrIncludeBooleanAttr(!unref(isReasonValid) || unref(loading)) ? " disabled" : ""} class="${ssrRenderClass([unref(isLateMode) ? "bg-yellow-600 hover:bg-yellow-700 disabled:bg-yellow-400" : "bg-primary-600 hover:bg-primary-700 disabled:bg-primary-400", "px-4 py-2 text-sm font-medium text-white rounded-lg transition-colors disabled:cursor-not-allowed"])}">`);
          if (unref(loading)) {
            _push2(`<span class="flex items-center gap-2">`);
            _push2(ssrRenderComponent(unref(Loader2), { class: "w-4 h-4 animate-spin" }, null, _parent));
            _push2(` Отправка... </span>`);
          } else {
            _push2(`<span>${ssrInterpolate(unref(isLateMode) ? "Подтвердить отметку" : "Отправить запрос")}</span>`);
          }
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/attendance/LateMarkingModal.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const LateMarkingModal = Object.assign(_sfc_main$1, { __name: "AttendanceLateMarkingModal" });
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getRetakeInfo = (studentId, currentEventId) => {
    ...
    const checkMarkingAccess = async () => {
      ...
      try {
        const response = await authFetch(`/api/attendance/marking/check/${selectedEventId.value}`);
        if (response.success) {
          markingAccess.value = response.access;
        }
      } catch (error2) {
        console.error("Error checking access:", error2);
        markingAccess.value = {
          allowed: true,
          status: "allowed",
          deadline: "",
    ...
    const handleRequestApproval = async (reason) => {
      try {
        ...
        toast.error(error2.data?.message || "Ошибка отправки запроса");
      }
    };
    const saveBulkAttendanceWithReason = async (lateReason) => {
      ...
      try {
        const eligibleRows = selectedEvent.value.scheduleEvent.isRetake && selectedEvent.value.scheduleEvent.allowedStudentIds ? rows.value.filter(
          (row) => selectedEvent.value.scheduleEvent.allowedStudentIds.includes(
            row.student.id
          )
        ) : rows.value;
        ...
        if (error2?.statusCode === 403 || error2?.status === 403) {
          const message = error2.data?.message || error2.message || "Доступ запрещён";
          if (message.includes("одобрение") || message.includes("Требуется одобрение")) {
            toast.warning(message);
            showBulkAttendanceModal.value = false;
            showLateMarkingModal.value = true;
          ...
          toast.error(error2.data?.message || error2.message || "Ошибка сохранения");
        }
      } finally {
    ...
    const saveBulkFinalGrade = async () => {
      ...
      if (bulkFinalGradeMode.value === "custom" && (bulkFinalGradeValue.value < 0 || bulkFinalGradeValue.value > 100)) {
        toast.error("Оценка должна быть от 0 до 100");
        return;
      }
      bulkSaving.value = true;
      try {
        ...
        const response = await authFetch("/api/final-grades/bulk", {
          method: "POST",
          body: {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        _push(` Журнал посещаемости и оценок </p></div></div><div class="flex items-center gap-3"><div class="hidden lg:flex items-center gap-4 bg-gray-100 dark:bg-gray-800 rounded-lg px-4 py-2"><div class="text-center"><p class="text-xs text-gray-500 dark:text-gray-400"> Студентов </p><p class="text-lg font-bold text-gray-900 dark:text-white">${ssrInterpolate(unref(summary)?.totalStudents || 0)}</p></div><div class="w-px h-8 bg-gray-300 dark:bg-gray-600"></div><div class="text-center"><p class="text-xs text-gray-500 dark:text-gray-400">Занятий</p><p class="text-lg font-bold text-gray-900 dark:text-white">${ssrInterpolate(unref(summary)?.totalEvents || 0)}</p></div><div class="w-px h-8 bg-gray-300 dark:bg-gray-600"></div><div class="text-center"><p class="text-xs text-gray-500 dark:text-gray-400"> Ср. посещ. </p><p class="${ssrRenderClass([getAttendanceColor(unref(summary)?.averageAttendance || 0), "text-lg font-bold"])}">${ssrInterpolate((unref(summary)?.averageAttendance || 0).toFixed(1))}% </p></div></div>`);
        if (unref(columns).length > 0) {
          _push(`<div class="relative inline-block"><button${ssrIncludeBooleanAttr(unref(generatingPdf)) ? " disabled" : ""} class="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title="Скачать ведомость">`);
          if (!unref(generatingPdf)) {
            _push(`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>`);
          } else {
            _push(`<svg class="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>`);
          }
          _push(`<span class="hidden sm:inline">${ssrInterpolate(unref(generatingPdf) ? "Генерация..." : "Ведомость")}</span><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>`);
          if (unref(showReportDropdown)) {
            _push(`<div class="absolute right-0 mt-2 w-48 bg-white dark:bg-boxdark rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50"><button class="w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center gap-2 rounded-t-lg text-gray-900 dark:text-white"><svg class="w-4 h-4 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><span>Скачать PDF</span></button><button class="w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center gap-2 rounded-b-lg text-gray-900 dark:text-white"><svg class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><span>Скачать Word</span></button></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        ...
        if (unref(columns).length > 0 && !unref(isArchived)) {
          ...
          ssrRenderList(unref(columns), (col) => {
            ...
            ))} ${ssrInterpolate(col.hasGrade ? "(с оценкой)" : "")}</option>`);
          });
          _push(`<!--]--></select>`);
          if (unref(selectedEventId) && unref(markingAccess)) {
            _push(`<div class="flex items-center gap-2">`);
            if (unref(accessLoading)) {
              _push(`<span class="text-gray-400"><svg class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></span>`);
            } else if (unref(markingAccess).status === "allowed") {
              _push(`<span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Доступно </span>`);
            } else if (unref(markingAccess).status === "late") {
              _push(`<span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg> Опоздание </span>`);
            } else if (unref(markingAccess).status === "requires_approval") {
              _push(`<span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg> Требует одобрения </span>`);
            } else if (unref(markingAccess).status === "denied") {
              _push(`<span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path></svg> Недоступно </span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div>`);
          ...
          _push(ssrRenderComponent(_component_UiButton, {
            variant: "primary",
            size: "sm",
            disabled: !unref(selectedEventId) || unref(markingAccess)?.status === "denied",
          ...
          _push(ssrRenderComponent(_component_UiButton, {
            ...
            onClick: openBulkFinalGradeModal
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"${_scopeId}></path></svg> Итоговые оценки `);
              } else {
                return [
                  (openBlock(), createBlock("svg", {
                    class: "w-4 h-4 mr-1",
                  ...
                  createTextVNode(" Итоговые оценки ")
                ];
              }
            }),
            _: 1
          ...
          _push(`</div></div>`);
          if (unref(markingAccess) && unref(markingAccess).status === "late") {
            _push(`<div class="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800"><div class="flex items-start gap-2"><svg class="w-5 h-5 text-yellow-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg><p class="text-sm text-yellow-700 dark:text-yellow-300"> Срок отметки для этого занятия истёк. Отметка будет сохранена с пометкой «Опоздание». </p></div></div>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(markingAccess) && unref(markingAccess).status === "requires_approval") {
            _push(`<div class="mt-3 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800"><div class="flex items-start gap-2"><svg class="w-5 h-5 text-red-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg><div><p class="text-sm font-medium text-red-700 dark:text-red-300"> Требуется одобрение администратора </p><p class="text-xs text-red-600 dark:text-red-400 mt-1"> Срок отметки для этого занятия давно истёк. Нажмите «Отметить всех» чтобы отправить запрос на одобрение. </p></div></div></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        ...
        if (unref(columns).length > 0) {
          ...
          ssrRenderList(unref(columns), (column) => {
            _push(`<th class="px-2 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider min-w-[80px]"><div class="flex flex-col items-center gap-1"><div class="flex items-center gap-1"><span class="${ssrRenderClass([{
              "bg-purple-500": column.scheduleEvent.isRetake,
              "bg-blue-500": !column.scheduleEvent.isRetake && column.scheduleEvent.eventType === "theory",
              "bg-green-500": !column.scheduleEvent.isRetake && column.scheduleEvent.eventType === "practice",
              "bg-orange-500": !column.scheduleEvent.isRetake && column.scheduleEvent.eventType === "assessment",
              "bg-gray-500": !column.scheduleEvent.isRetake && column.scheduleEvent.eventType === "other"
            }, "inline-block w-2 h-2 rounded-full"])}"></span>`);
            if (column.scheduleEvent.isRetake) {
              _push(`<span class="inline-flex items-center justify-center w-4 h-4 text-[8px] font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded" title="Режим пересдачи"> П </span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</div><span class="text-xs">${ssrInterpolate(formatColumnDate(column.scheduleEvent.date))}</span><span class="text-[10px] text-gray-400">${ssrInterpolate(formatTimeRange(
          ...
          ssrRenderList(unref(rows), (row) => {
            _push(`<tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"><td class="sticky left-0 z-10 bg-white dark:bg-boxdark px-4 py-3"><div class="flex items-center gap-3"><div class="shrink-0 flex h-8 w-8 items-center justify-center rounded-full bg-success/10 text-success text-sm font-semibold">${ssrInterpolate(getInitials(row.student.fullName))}</div><span class="font-medium text-gray-900 dark:text-white text-sm"${ssrRenderAttr("title", row.student.fullName)}>${ssrInterpolate(row.student.fullName)}</span></div></td><!--[-->`);
            ssrRenderList(row.cells, (cell, cellIndex) => {
              ...
              if (unref(columns)[cellIndex]) {
                _push(ssrRenderComponent(AttendanceCell, {
                  ...
                  "is-retake-target": getRetakeInfo(
                    row.student.id,
                    unref(columns)[cellIndex].scheduleEvent.id
                  ).isRetakeTarget,
                  "replacement-grade": getRetakeInfo(
                    row.student.id,
                    unref(columns)[cellIndex].scheduleEvent.id
                  ).replacementGrade,
                  "read-only": unref(isArchived),
          ...
          _push(`<!--]--></tbody></table></div><div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400"><span class="font-medium">Типы занятий:</span><span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-blue-500"></span> Теория </span><span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-green-500"></span> Практика (с оценкой) </span><span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-orange-500"></span> Проверка знаний (с оценкой) </span><span class="flex items-center gap-1"><span class="w-3 h-3 rounded-full bg-purple-500"></span> Пересдача </span><span class="mx-2 text-gray-300 dark:text-gray-600">|</span><span class="font-medium">Оценки:</span><span class="flex items-center gap-1" title="Автоматическая оценка из теста"><span class="relative w-5 h-5 rounded bg-success/20 flex items-center justify-center text-xs text-success"><span class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-blue-500"></span> 85 </span> Из теста </span><span class="flex items-center gap-1" title="Ручная оценка за практику"><span class="relative w-5 h-5 rounded bg-success/20 flex items-center justify-center text-xs text-success"> 90 </span> Ручная </span><span class="flex items-center gap-1" title="Изменённая оценка"><span class="relative w-5 h-5 rounded bg-purple-200 dark:bg-purple-900/40 flex items-center justify-center text-xs text-purple-700 dark:text-purple-300 ring-1 ring-purple-400"><span class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-purple-500"></span> 78 </span> Изменена </span></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--]-->`);
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (unref(selectedEvent)) {
              _push2(`<div${_scopeId}><p class="text-sm text-gray-600 dark:text-gray-400 mb-4"${_scopeId}> Занятие: ${ssrInterpolate(formatColumnDate(unref(selectedEvent).scheduleEvent.date))} ${ssrInterpolate(formatTimeRange(
                unref(selectedEvent).scheduleEvent.startTime,
                unref(selectedEvent).scheduleEvent.endTime
              ))} (${ssrInterpolate(unref(selectedEvent).scheduleEvent.academicHours)} а-ч) </p><div${_scopeId}><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"${_scopeId}> Часы посещения для всех (из ${ssrInterpolate(unref(selectedEvent).scheduleEvent.academicHours)}) </label><div class="flex items-center gap-2"${_scopeId}><input${ssrRenderAttr("value", unref(bulkAttendanceHours))} type="number" step="0.5" min="0"${ssrRenderAttr("max", unref(selectedEvent).scheduleEvent.academicHours)} class="flex-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-boxdark px-4 py-2 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50"${_scopeId}><span class="text-gray-500"${_scopeId}>а-ч</span></div><div class="flex gap-2 mt-3"${_scopeId}><button class="${ssrRenderClass([
                unref(bulkAttendanceHours) === 0 ? "bg-primary text-white border-primary" : "border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700",
                "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
              ])}"${_scopeId}> Никто (0) </button><button class="${ssrRenderClass([
                unref(bulkAttendanceHours) === unref(selectedEvent).scheduleEvent.academicHours ? "bg-primary text-white border-primary" : "border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700",
                "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
              ])}"${_scopeId}> Все (${ssrInterpolate(unref(selectedEvent).scheduleEvent.academicHours)}) </button></div></div><div class="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4"${_scopeId}><p class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}> Будет отмечено ${ssrInterpolate(getEligibleStudentsCount())} слушателей `);
              if (unref(selectedEvent).scheduleEvent.isRetake) {
                _push2(`<span class="text-purple-600 dark:text-purple-400 font-medium"${_scopeId}> (только участники пересдачи) </span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</p>`);
              if (unref(selectedEvent).scheduleEvent.isRetake && unref(selectedEvent).scheduleEvent.allowedStudentIds) {
                _push2(`<div class="mt-3"${_scopeId}><p class="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2"${_scopeId}> Участники пересдачи: </p><div class="max-h-32 overflow-y-auto bg-purple-50 dark:bg-purple-900/20 rounded-lg p-2 border border-purple-200 dark:border-purple-800"${_scopeId}><div class="space-y-1"${_scopeId}><!--[-->`);
                ssrRenderList(getEligibleStudents(), (student) => {
                  _push2(`<div class="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2"${_scopeId}><span class="w-1.5 h-1.5 rounded-full bg-purple-500"${_scopeId}></span> ${ssrInterpolate(student.fullName)}</div>`);
                });
                _push2(`<!--]--></div></div></div>`);
              } else {
                _push2(`<!---->`);
            ...
            return [
              createVNode("div", { class: "space-y-4" }, [
                unref(selectedEvent) ? (openBlock(), createBlock("div", { key: 0 }, [
                  createVNode("p", { class: "text-sm text-gray-600 dark:text-gray-400 mb-4" }, " Занятие: " + toDisplayString(formatColumnDate(unref(selectedEvent).scheduleEvent.date)) + " " + toDisplayString(formatTimeRange(
                    unref(selectedEvent).scheduleEvent.startTime,
                    unref(selectedEvent).scheduleEvent.endTime
                  )) + " (" + toDisplayString(unref(selectedEvent).scheduleEvent.academicHours) + " а-ч) ", 1),
                  createVNode("div", null, [
                  ...
                  createVNode("div", { class: "pt-4 border-t border-gray-200 dark:border-gray-700 mt-4" }, [
                    createVNode("p", { class: "text-sm text-gray-500 dark:text-gray-400" }, [
                      createTextVNode(" Будет отмечено " + toDisplayString(getEligibleStudentsCount()) + " слушателей ", 1),
                      unref(selectedEvent).scheduleEvent.isRetake ? (openBlock(), createBlock("span", {
                        key: 0,
                        class: "text-purple-600 dark:text-purple-400 font-medium"
                      }, " (только участники пересдачи) ")) : createCommentVNode("", true)
                    ]),
                    unref(selectedEvent).scheduleEvent.isRetake && unref(selectedEvent).scheduleEvent.allowedStudentIds ? (openBlock(), createBlock("div", {
                      key: 0,
                      class: "mt-3"
                    }, [
                      createVNode("p", { class: "text-xs font-medium text-gray-700 dark:text-gray-300 mb-2" }, " Участники пересдачи: "),
                      createVNode("div", { class: "max-h-32 overflow-y-auto bg-purple-50 dark:bg-purple-900/20 rounded-lg p-2 border border-purple-200 dark:border-purple-800" }, [
                        createVNode("div", { class: "space-y-1" }, [
                          (openBlock(true), createBlock(Fragment, null, renderList(getEligibleStudents(), (student) => {
                            return openBlock(), createBlock("div", {
                              key: student.id,
                              class: "text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2"
                            }, [
                              createVNode("span", { class: "w-1.5 h-1.5 rounded-full bg-purple-500" }),
                              createTextVNode(" " + toDisplayString(student.fullName), 1)
                            ]);
                          }), 128))
                        ])
                      ])
          ...
          if (_push2) {
            ...
            if (unref(selectedEvent)) {
              _push2(`<div${_scopeId}><p class="text-sm text-gray-600 dark:text-gray-400 mb-4"${_scopeId}> Занятие: ${ssrInterpolate(formatColumnDate(unref(selectedEvent).scheduleEvent.date))} ${ssrInterpolate(formatTimeRange(
                unref(selectedEvent).scheduleEvent.startTime,
                unref(selectedEvent).scheduleEvent.endTime
              ))}</p><div${_scopeId}><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"${_scopeId}> Оценка для всех (0-100) </label><input${ssrRenderAttr("value", unref(bulkGradeValue))} type="number" min="0" max="100" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-boxdark px-4 py-2 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50"${_scopeId}><div class="flex flex-wrap gap-2 mt-3"${_scopeId}><!--[-->`);
              ssrRenderList([100, 90, 80, 70, 60], (grade) => {
                _push2(`<button class="${ssrRenderClass([
                  unref(bulkGradeValue) === grade ? "bg-primary text-white border-primary" : "border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700",
                  "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
                ])}"${_scopeId}>${ssrInterpolate(grade)}</button>`);
              });
              _push2(`<!--]--></div></div><div class="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4"${_scopeId}><p class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}> Будет выставлено ${ssrInterpolate(getEligibleStudentsCount())} оценок `);
              if (unref(selectedEvent).scheduleEvent.isRetake) {
                _push2(`<span class="text-purple-600 dark:text-purple-400 font-medium"${_scopeId}> (только участники пересдачи) </span>`);
              } else {
                _push2(`<!---->`);
            ...
            return [
              createVNode("div", { class: "space-y-4" }, [
                unref(selectedEvent) ? (openBlock(), createBlock("div", { key: 0 }, [
                  ...
                  createVNode("div", { class: "pt-4 border-t border-gray-200 dark:border-gray-700 mt-4" }, [
                    createVNode("p", { class: "text-sm text-gray-500 dark:text-gray-400" }, [
                      createTextVNode(" Будет выставлено " + toDisplayString(getEligibleStudentsCount()) + " оценок ", 1),
                      unref(selectedEvent).scheduleEvent.isRetake ? (openBlock(), createBlock("span", {
                        key: 0,
                        class: "text-purple-600 dark:text-purple-400 font-medium"
                      }, " (только участники пересдачи) ")) : createCommentVNode("", true)
      ...
      _push(ssrRenderComponent(_component_UiModal, {
        "is-open": unref(showBulkFinalGradeModal),
        title: "Массовое выставление итоговых оценок",
        size: "lg",
        onClose: ($event) => showBulkFinalGradeModal.value = false
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="space-y-4"${_scopeId}><p class="text-sm text-gray-600 dark:text-gray-400"${_scopeId}> Выставить итоговые оценки всем слушателям по дисциплине <span class="font-medium text-gray-900 dark:text-white"${_scopeId}>${ssrInterpolate(unref(disciplineName))}</span></p><div class="space-y-3"${_scopeId}><label class="block text-sm font-medium text-gray-700 dark:text-gray-300"${_scopeId}> Режим выставления </label><div class="space-y-2"${_scopeId}><label class="${ssrRenderClass([
              unref(bulkFinalGradeMode) === "average" ? "border-primary bg-primary/5" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800",
              "flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors"
            ])}"${_scopeId}><input${ssrIncludeBooleanAttr(ssrLooseEqual(unref(bulkFinalGradeMode), "average")) ? " checked" : ""} type="radio" value="average" class="mt-0.5"${_scopeId}><div${_scopeId}><p class="font-medium text-gray-900 dark:text-white"${_scopeId}> По средней оценке </p><p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5"${_scopeId}> Итоговая оценка = средняя оценка за занятия. Статус определяется автоматически. </p></div></label><label class="${ssrRenderClass([
              unref(bulkFinalGradeMode) === "custom" ? "border-primary bg-primary/5" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800",
              "flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors"
            ])}"${_scopeId}><input${ssrIncludeBooleanAttr(ssrLooseEqual(unref(bulkFinalGradeMode), "custom")) ? " checked" : ""} type="radio" value="custom" class="mt-0.5"${_scopeId}><div${_scopeId}><p class="font-medium text-gray-900 dark:text-white"${_scopeId}> Единая оценка всем </p><p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5"${_scopeId}> Выставить одинаковую оценку всем слушателям. </p></div></label></div></div>`);
            if (unref(bulkFinalGradeMode) === "custom") {
              _push2(`<div class="space-y-2"${_scopeId}><label class="block text-sm font-medium text-gray-700 dark:text-gray-300"${_scopeId}> Оценка для всех (0-100) </label><input${ssrRenderAttr("value", unref(bulkFinalGradeValue))} type="number" min="0" max="100" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-boxdark px-4 py-2 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50"${_scopeId}><div class="flex flex-wrap gap-2"${_scopeId}><!--[-->`);
              ssrRenderList([100, 90, 80, 70, 60], (grade) => {
                _push2(`<button class="${ssrRenderClass([
                  unref(bulkFinalGradeValue) === grade ? "bg-primary text-white border-primary" : "border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700",
                  "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
                ])}"${_scopeId}>${ssrInterpolate(grade)}</button>`);
              });
              _push2(`<!--]--></div></div>`);
            ...
            _push2(`<div class="space-y-2"${_scopeId}><label class="block text-sm font-medium text-gray-700 dark:text-gray-300"${_scopeId}> Порог для статуса &quot;Сдано&quot; (%) </label><div class="flex items-center gap-3"${_scopeId}><input${ssrRenderAttr("value", unref(bulkFinalGradeThreshold))} type="number" min="0" max="100" class="w-24 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-boxdark px-3 py-2 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50"${_scopeId}><span class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}> Оценка ≥ ${ssrInterpolate(unref(bulkFinalGradeThreshold))} → &quot;Сдано&quot;, иначе → &quot;Не сдано&quot; </span></div></div><div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4"${_scopeId}><h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3"${_scopeId}> Предпросмотр (${ssrInterpolate(unref(rows).length)} слушателей) </h4><div class="max-h-48 overflow-y-auto bg-gray-50 dark:bg-gray-800 rounded-lg p-3"${_scopeId}><div class="space-y-2"${_scopeId}><!--[-->`);
            ssrRenderList(unref(rows), (row) => {
              _push2(`<div class="flex items-center justify-between text-sm"${_scopeId}><span class="text-gray-900 dark:text-white"${_scopeId}>${ssrInterpolate(row.student.fullName)}</span><div class="flex items-center gap-2"${_scopeId}><span class="${ssrRenderClass([
                getPreviewGrade(row) >= unref(bulkFinalGradeThreshold) ? "bg-success/10 text-success" : "bg-danger/10 text-danger",
                "px-2 py-0.5 rounded text-xs font-medium"
              ])}"${_scopeId}>${ssrInterpolate(getPreviewGrade(row))}</span><span class="${ssrRenderClass([
                getPreviewGrade(row) >= unref(bulkFinalGradeThreshold) ? "text-success" : "text-danger",
                "text-xs"
              ])}"${_scopeId}>${ssrInterpolate(getPreviewGrade(row) >= unref(bulkFinalGradeThreshold) ? "Сдано" : "Не сдано")}</span></div></div>`);
            });
            _push2(`<!--]--></div></div></div><div class="flex justify-end gap-3 pt-4"${_scopeId}>`);
            _push2(ssrRenderComponent(_component_UiButton, {
              variant: "outline",
              onClick: ($event) => showBulkFinalGradeModal.value = false
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(` Отмена `);
            ...
            _push2(ssrRenderComponent(_component_UiButton, {
              loading: unref(bulkSaving),
              onClick: saveBulkFinalGrade
            }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(` Выставить итоговые оценки `);
                } else {
                  return [
                    createTextVNode(" Выставить итоговые оценки ")
                  ];
                }
              }),
              _: 1
            ...
            return [
              createVNode("div", { class: "space-y-4" }, [
                createVNode("p", { class: "text-sm text-gray-600 dark:text-gray-400" }, [
                  createTextVNode(" Выставить итоговые оценки всем слушателям по дисциплине "),
                  createVNode("span", { class: "font-medium text-gray-900 dark:text-white" }, toDisplayString(unref(disciplineName)), 1)
                ]),
                createVNode("div", { class: "space-y-3" }, [
                  createVNode("label", { class: "block text-sm font-medium text-gray-700 dark:text-gray-300" }, " Режим выставления "),
                  createVNode("div", { class: "space-y-2" }, [
                    createVNode("label", {
                      ...
                      withDirectives(createVNode("input", {
                        "onUpdate:modelValue": ($event) => isRef(bulkFinalGradeMode) ? bulkFinalGradeMode.value = $event : null,
                        type: "radio",
                        value: "average",
                        class: "mt-0.5"
                      }, null, 8, ["onUpdate:modelValue"]), [
                        [vModelRadio, unref(bulkFinalGradeMode)]
                      ]),
                      createVNode("div", null, [
                        createVNode("p", { class: "font-medium text-gray-900 dark:text-white" }, " По средней оценке "),
                        createVNode("p", { class: "text-xs text-gray-500 dark:text-gray-400 mt-0.5" }, " Итоговая оценка = средняя оценка за занятия. Статус определяется автоматически. ")
                      ])
                    ], 2),
                    createVNode("label", {
                      class: [
                      ...
                      createVNode("div", null, [
                        createVNode("p", { class: "font-medium text-gray-900 dark:text-white" }, " Единая оценка всем "),
                        createVNode("p", { class: "text-xs text-gray-500 dark:text-gray-400 mt-0.5" }, " Выставить одинаковую оценку всем слушателям. ")
                      ])
                    ], 2)
                  ])
                ]),
                unref(bulkFinalGradeMode) === "custom" ? (openBlock(), createBlock("div", {
                  key: 0,
                  class: "space-y-2"
                }, [
                  createVNode("label", { class: "block text-sm font-medium text-gray-700 dark:text-gray-300" }, " Оценка для всех (0-100) "),
                  withDirectives(createVNode("input", {
                    "onUpdate:modelValue": ($event) => isRef(bulkFinalGradeValue) ? bulkFinalGradeValue.value = $event : null,
                    type: "number",
                    min: "0",
                    max: "100",
                    class: "w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-boxdark px-4 py-2 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50"
                  ...
                  createVNode("div", { class: "flex flex-wrap gap-2" }, [
                    (openBlock(), createBlock(Fragment, null, renderList([100, 90, 80, 70, 60], (grade) => {
                      return createVNode("button", {
                        ...
                        onClick: ($event) => bulkFinalGradeValue.value = grade
                      }, toDisplayString(grade), 11, ["onClick"]);
                    }), 64))
                  ])
                ])) : createCommentVNode("", true),
                createVNode("div", { class: "space-y-2" }, [
                  createVNode("label", { class: "block text-sm font-medium text-gray-700 dark:text-gray-300" }, ' Порог для статуса "Сдано" (%) '),
                  createVNode("div", { class: "flex items-center gap-3" }, [
                    withDirectives(createVNode("input", {
                      "onUpdate:modelValue": ($event) => isRef(bulkFinalGradeThreshold) ? bulkFinalGradeThreshold.value = $event : null,
                      type: "number",
                      min: "0",
                      max: "100",
                      ...
                      [
                        vModelText,
                        unref(bulkFinalGradeThreshold),
                        void 0,
                        { number: true }
                      ]
                    ]),
                    createVNode("span", { class: "text-sm text-gray-500 dark:text-gray-400" }, " Оценка ≥ " + toDisplayString(unref(bulkFinalGradeThreshold)) + ' → "Сдано", иначе → "Не сдано" ', 1)
                  ])
                ]),
                createVNode("div", { class: "border-t border-gray-200 dark:border-gray-700 pt-4 mt-4" }, [
                  createVNode("h4", { class: "text-sm font-medium text-gray-700 dark:text-gray-300 mb-3" }, " Предпросмотр (" + toDisplayString(unref(rows).length) + " слушателей) ", 1),
                  createVNode("div", { class: "max-h-48 overflow-y-auto bg-gray-50 dark:bg-gray-800 rounded-lg p-3" }, [
                    createVNode("div", { class: "space-y-2" }, [
                      (openBlock(true), createBlock(Fragment, null, renderList(unref(rows), (row) => {
                        return openBlock(), createBlock("div", {
                          key: row.student.id,
                          class: "flex items-center justify-between text-sm"
                        }, [
                          createVNode("span", { class: "text-gray-900 dark:text-white" }, toDisplayString(row.student.fullName), 1),
                          createVNode("div", { class: "flex items-center gap-2" }, [
                            createVNode("span", {
                              class: [
                                "px-2 py-0.5 rounded text-xs font-medium",
                            ...
                            }, toDisplayString(getPreviewGrade(row)), 3),
                            createVNode("span", {
                              class: [
                                "text-xs",
                                getPreviewGrade(row) >= unref(bulkFinalGradeThreshold) ? "text-success" : "text-danger"
                              ]
                            }, toDisplayString(getPreviewGrade(row) >= unref(bulkFinalGradeThreshold) ? "Сдано" : "Не сдано"), 3)
                          ])
                        ]);
                      }), 128))
                    ])
      ...
      _push(ssrRenderComponent(LateMarkingModal, {
        modelValue: unref(showLateMarkingModal),
        "onUpdate:modelValue": ($event) => isRef(showLateMarkingModal) ? showLateMarkingModal.value = $event : null,
        "event-title": unref(selectedEvent)?.scheduleEvent.title || "",
        "event-date": unref(selectedEvent)?.scheduleEvent.startTime || "",
        deadline: unref(markingAccess)?.deadline || "",
        status: unref(markingAccess)?.status === "requires_approval" ? "requires_approval" : "late",
        onConfirm: handleLateMarkingConfirm,
        onRequestApproval: handleRequestApproval,
        onCancel: ($event) => showLateMarkingModal.value = false
      }, null, _parent));
      _push(`</div>`);
    };
  }
...
//# sourceMappingURL=_slug_-D2XPPyLq.mjs.map

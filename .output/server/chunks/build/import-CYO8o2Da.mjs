...
import { ssrRenderAttrs, ssrRenderList, ssrRenderClass, ssrInterpolate, ssrRenderComponent, ssrIncludeBooleanAttr, ssrRenderVNode, ssrRenderStyle, ssrRenderSlot } from 'vue/server-renderer';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(steps, (step) => {
        ...
        _push(`</div><span class="${ssrRenderClass([
          step.id <= unref(store).currentStep ? "text-primary" : "text-gray-500 dark:text-gray-400",
          "mt-2 text-xs font-medium text-center"
        ])}">${ssrInterpolate(step.label)}</span>`);
        if (step.id !== steps.length) {
          _push(`<div class="absolute top-5 left-[50%] w-full h-[2px] -z-10"><div class="${ssrRenderClass([
            step.id < unref(store).currentStep ? "bg-primary" : "bg-gray-200 dark:bg-gray-700",
      ...
      if (unref(store).error) {
        _push(`<div class="mb-6 rounded-lg border border-danger/20 bg-danger/5 dark:bg-danger/10 p-4 shadow-sm"><div class="flex items-center gap-3"><div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-danger"><svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div><div class="flex-1 min-w-0"><h4 class="font-semibold text-danger dark:text-danger">Ошибка</h4><p class="text-sm text-danger/80 dark:text-danger/90">${ssrInterpolate(unref(store).error)}</p></div><button class="text-danger hover:text-danger/80 dark:hover:text-danger/70 transition-colors shrink-0"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="rounded-lg border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">`);
      if (unref(store).currentStep === 1) {
        _push(`<div class="p-6 lg:p-8">`);
        ...
        if (unref(store).analysis) {
          ...
          _push(`<div class="text-center py-12"><div class="mb-4"><svg class="mx-auto h-16 w-16 text-gray-400 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg></div><p class="text-gray-500 dark:text-gray-400 mb-4"> Файл не выбран или данные анализа потеряны. Пожалуйста, начните заново. </p><button class="px-6 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-600 transition-colors font-medium"> Вернуться к загрузке </button></div>`);
        }
        _push(`</div>`);
        ...
        if (unref(store).progress) {
          ...
          _push(`<div class="flex flex-col items-center justify-center py-16"><div class="animate-spin rounded-full h-16 w-16 border-4 border-gray-200 dark:border-gray-700 border-t-primary mb-6"></div><p class="text-gray-600 dark:text-gray-400 font-medium"> Инициализация импорта... </p><p class="text-sm text-gray-500 dark:text-gray-500 mt-2"> Пожалуйста, подождите </p></div>`);
        }
        _push(`</div>`);
      } else if (unref(store).currentStep === 4) {
        _push(`<div class="p-6 lg:p-8">`);
        if (unref(store).progress) {
...
//# sourceMappingURL=import-CYO8o2Da.mjs.map

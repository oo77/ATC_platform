...
import { e as useAuth, g as getRolePermissions, U as UserRole, P as Permission, f as roleHasPermission, h as roleHasAnyPermission, i as roleHasAllPermissions } from './server.mjs';
...
function usePermissions() {
  ...
  const canViewAllStudents = computed(
    () => hasPermission(Permission.STUDENTS_VIEW_ALL)
  );
  const canCreateStudents = computed(
    () => hasPermission(Permission.STUDENTS_CREATE)
  );
  const canEditStudents = computed(
    () => hasPermission(Permission.STUDENTS_UPDATE)
  );
  const canDeleteStudents = computed(
    () => hasPermission(Permission.STUDENTS_DELETE)
  );
  const canImportStudents = computed(
    () => hasPermission(Permission.STUDENTS_IMPORT)
  );
  const canExportStudents = computed(
    () => hasPermission(Permission.STUDENTS_EXPORT)
  );
  const canViewInstructors = computed(
    () => hasPermission(Permission.INSTRUCTORS_VIEW)
  );
  const canCreateInstructors = computed(
    () => hasPermission(Permission.INSTRUCTORS_CREATE)
  );
  const canEditInstructors = computed(
    () => hasPermission(Permission.INSTRUCTORS_UPDATE)
  );
  const canDeleteInstructors = computed(
    () => hasPermission(Permission.INSTRUCTORS_DELETE)
  );
  const canViewInstructorHours = computed(
    () => hasPermission(Permission.INSTRUCTORS_HOURS)
  );
  const canViewOrganizations = computed(
    () => hasPermission(Permission.ORGANIZATIONS_VIEW)
  );
  const canCreateOrganizations = computed(
    () => hasPermission(Permission.ORGANIZATIONS_CREATE)
  );
  const canEditOrganizations = computed(
    () => hasPermission(Permission.ORGANIZATIONS_UPDATE)
  );
  const canDeleteOrganizations = computed(
    () => hasPermission(Permission.ORGANIZATIONS_DELETE)
  );
  const canViewRepresentatives = computed(
    () => hasPermission(Permission.REPRESENTATIVES_VIEW)
  );
  const canApproveRepresentatives = computed(
    () => hasPermission(Permission.REPRESENTATIVES_APPROVE)
  );
  const canBlockRepresentatives = computed(
    () => hasPermission(Permission.REPRESENTATIVES_BLOCK)
  );
  const canManageRepresentatives = computed(
    () => hasPermission(Permission.REPRESENTATIVES_MANAGE)
  );
  const canViewCourses = computed(() => hasPermission(Permission.COURSES_VIEW));
  const canCreateCourses = computed(
    () => hasPermission(Permission.COURSES_CREATE)
  );
  const canEditCourses = computed(
    () => hasPermission(Permission.COURSES_UPDATE)
  );
  const canDeleteCourses = computed(
    () => hasPermission(Permission.COURSES_DELETE)
  );
  const canArchiveCourses = computed(
    () => hasPermission(Permission.COURSES_ARCHIVE)
  );
  const canViewDisciplines = computed(
    () => hasPermission(Permission.DISCIPLINES_VIEW)
  );
  const canManageDisciplines = computed(
    () => hasPermission(Permission.DISCIPLINES_MANAGE)
  );
  const canViewGroups = computed(
    () => hasAnyPermission([Permission.GROUPS_VIEW_ALL, Permission.GROUPS_VIEW_OWN])
  );
  const canViewAllGroups = computed(
    () => hasPermission(Permission.GROUPS_VIEW_ALL)
  );
  ...
  const canViewAllSchedule = computed(
    () => hasPermission(Permission.SCHEDULE_VIEW_ALL)
  );
  const canCreateSchedule = computed(
    () => hasPermission(Permission.SCHEDULE_CREATE)
  );
  const canEditSchedule = computed(
    () => hasPermission(Permission.SCHEDULE_UPDATE)
  );
  const canDeleteSchedule = computed(
    () => hasPermission(Permission.SCHEDULE_DELETE)
  );
  const canViewAttendance = computed(
    () => hasPermission(Permission.ATTENDANCE_VIEW)
  );
  const canMarkAttendance = computed(
    () => hasPermission(Permission.ATTENDANCE_MARK)
  );
  const canEditAttendance = computed(
    () => hasPermission(Permission.ATTENDANCE_EDIT)
  );
  const canViewGrades = computed(() => hasPermission(Permission.GRADES_VIEW));
  const canManageGrades = computed(
    () => hasPermission(Permission.GRADES_MANAGE)
  );
  ...
  const canIssueCertificates = computed(
    () => hasPermission(Permission.CERTIFICATES_ISSUE)
  );
  const canRevokeCertificates = computed(
    () => hasPermission(Permission.CERTIFICATES_REVOKE)
  );
  const canDownloadCertificates = computed(
    () => hasPermission(Permission.CERTIFICATES_DOWNLOAD)
  );
  const canViewTemplates = computed(
    () => hasPermission(Permission.TEMPLATES_VIEW)
  );
  const canCreateTemplates = computed(
    () => hasPermission(Permission.TEMPLATES_CREATE)
  );
  const canEditTemplates = computed(
    () => hasPermission(Permission.TEMPLATES_UPDATE)
  );
  const canDeleteTemplates = computed(
  ...
  const canViewTestBanks = computed(
    () => hasPermission(Permission.TEST_BANKS_VIEW)
  );
  const canManageTestBanks = computed(
    () => hasPermission(Permission.TEST_BANKS_MANAGE)
  );
  const canViewTestTemplates = computed(
    () => hasPermission(Permission.TEST_TEMPLATES_VIEW)
  );
  const canManageTestTemplates = computed(
    () => hasPermission(Permission.TEST_TEMPLATES_MANAGE)
  );
  ...
  const canViewTestResults = computed(
...
//# sourceMappingURL=usePermissions-Dc3CsxMS.mjs.map

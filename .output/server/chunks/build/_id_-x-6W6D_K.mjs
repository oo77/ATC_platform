...
import { _ as _sfc_main$2 } from './LanguageSelectModal-CH8tSNVs.mjs';
  ...
  setup(__props) {
    ...
    const startPreview = async (language = null) => {
      try {
        ...
        const errorMessage = error2.response?._data?.message || error2.message || "Произошла ошибка при создании preview-сессии";
        notification.value = {
          show: true,
          type: "error",
          title: "Ошибка",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_TestsLanguageSelectModal = _sfc_main$2;
      ...
      if (loading.value) {
        ...
        _push(`</svg><span class="text-gray-900 dark:text-white">Перемешивать вопросы</span></div><div class="${ssrRenderClass([
          template.value.shuffle_options ? "bg-success/10" : "bg-gray-100 dark:bg-gray-800",
          "flex items-center gap-3 p-3 rounded-lg"
        ])}"><svg class="${ssrRenderClass([
          "w-5 h-5",
          template.value.shuffle_options ? "text-success" : "text-gray-400"
        ])}" fill="none" stroke="currentColor" viewBox="0 0 24 24">`);
        ...
        if (template.value.proctoring_enabled && template.value.proctoring_settings) {
          _push(`<div class="grid grid-cols-1 md:grid-cols-3 gap-4"><div class="${ssrRenderClass([
            template.value.proctoring_settings.blockTabSwitch ? "bg-danger/10" : "bg-gray-100 dark:bg-gray-800",
            "flex items-center gap-3 p-3 rounded-lg"
          ])}"><svg class="${ssrRenderClass([
            "w-5 h-5",
            template.value.proctoring_settings.blockTabSwitch ? "text-danger" : "text-gray-400"
          ])}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"></path></svg><span class="text-sm text-gray-900 dark:text-white">Блокировка вкладок</span></div><div class="${ssrRenderClass([
            template.value.proctoring_settings.blockCopyPaste ? "bg-danger/10" : "bg-gray-100 dark:bg-gray-800",
            "flex items-center gap-3 p-3 rounded-lg"
          ])}"><svg class="${ssrRenderClass([
            "w-5 h-5",
            template.value.proctoring_settings.blockCopyPaste ? "text-danger" : "text-gray-400"
          ])}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg><span class="text-sm text-gray-900 dark:text-white">Блокировка копирования</span></div><div class="${ssrRenderClass([
            template.value.proctoring_settings.blockRightClick ? "bg-danger/10" : "bg-gray-100 dark:bg-gray-800",
            "flex items-center gap-3 p-3 rounded-lg"
          ])}"><svg class="${ssrRenderClass([
            "w-5 h-5",
            template.value.proctoring_settings.blockRightClick ? "text-danger" : "text-gray-400"
        ...
        if (questionsLoading.value) {
          ...
          ssrRenderList(questions.value.slice(0, 10), (question, index) => {
            _push(`<div class="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800"><span class="shrink-0 w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary">${ssrInterpolate(index + 1)}</span><div class="grow min-w-0"><p class="text-sm text-gray-900 dark:text-white line-clamp-2">${ssrInterpolate(question.question_text)}</p><div class="flex items-center gap-2 mt-1"><span class="${ssrRenderClass([
              "inline-flex items-center rounded-full px-1.5 py-0.5 text-xs",
              difficultyClasses[question.difficulty]
            ])}">${ssrInterpolate(difficultyLabels[question.difficulty])}</span><span class="text-xs text-gray-500">${ssrInterpolate(question.points)} б.</span></div></div></div>`);
          });
          _push(`<!--]-->`);
        ...
        if (analyticsLoading.value) {
          ...
          if (analytics.value.summary.totalSessions === 0) {
            ...
            ssrRenderList(analytics.value.sessions.slice(0, 20), (session) => {
              ...
              _push(`</div></td><td class="py-3 text-gray-600 dark:text-gray-400">${ssrInterpolate(session.groupCode || "—")}</td><td class="py-3 text-center"><span class="${ssrRenderClass([
                session.passed ? "bg-success/10 text-success" : "bg-danger/10 text-danger",
                "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
              ])}">${ssrInterpolate(session.score)}% </span></td><td class="py-3 text-center">`);
              if (session.passed) {
            ...
            if (analytics.value.questionStats && analytics.value.questionStats.length > 0) {
              ...
              ssrRenderList(analytics.value.questionStats.slice(0, 10), (q, idx) => {
                _push(`<div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-meta-4 rounded-lg"><span class="w-6 h-6 rounded-full bg-gray-200 dark:bg-strokedark flex items-center justify-center text-xs font-medium text-gray-600 dark:text-gray-400">${ssrInterpolate(idx + 1)}</span><div class="flex-1 min-w-0"><p class="text-sm text-gray-900 dark:text-white truncate">${ssrInterpolate(q.questionText)}</p></div><div class="flex items-center gap-4 text-xs"><span class="${ssrRenderClass([
                  q.correctRate >= 70 ? "bg-success/10 text-success" : q.correctRate >= 40 ? "bg-warning/10 text-warning" : "bg-danger/10 text-danger",
                  "px-2 py-1 rounded"
                ])}">${ssrInterpolate(q.correctRate)}% правильно </span><span class="text-gray-500">${ssrInterpolate(q.totalAnswers)} ответов </span></div></div>`);
              });
              _push(`<!--]--></div></div>`);
            } else {
              _push(`<!---->`);
      ...
      _push(ssrRenderComponent(_component_TestsLanguageSelectModal, {
        ...
        "custom-languages": availableLanguagesForModal.value,
        onClose: ($event) => showLanguageModal.value = false,
...
//# sourceMappingURL=_id_-x-6W6D_K.mjs.map

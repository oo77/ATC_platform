import { g as defineEventHandler, j as getRouterParam, h as createError, r as readBody, f as executeQuery } from '../../../_/nitro.mjs';
import { a as getScheduleEventById, e as checkScheduleConflicts, f as updateScheduleEvent } from '../../../_/scheduleRepository.mjs';
...
import { f as formatDateOnly, a as formatDateForDisplay, d as dateToLocalIsoString, b as dateToLocalIso } from '../../../_/timeUtils.mjs';
import 'grammy';
import 'uuid';
import 'node:http';
import 'node:https';
...
const _id__put = defineEventHandler(async (event) => {
  try {
    ...
    let eventHours;
    if (body.academicHours) {
      eventHours = body.academicHours;
    } else {
      const eventMinutes = body.durationMinutes ? body.durationMinutes : (endTime.getTime() - startTime.getTime()) / (1e3 * 60);
    ...
    if (checkInstructor) {
      ...
      if (instructorChanged || timeChanged || body.academicHours !== void 0) {
        let oldEventHours = 0;
        if (!instructorChanged) {
          if (existing.academicHours) {
            ...
            const academicHourMinutes = await getAcademicHourMinutes();
            const oldMinutes = existing.durationMinutes || (existing.endTime.getTime() - existing.startTime.getTime()) / (1e3 * 60);
            oldEventHours = Math.ceil(oldMinutes / academicHourMinutes);
          }
        }
        const additionalHours = instructorChanged ? eventHours : Math.max(0, eventHours - oldEventHours);

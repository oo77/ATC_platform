import { g as defineEventHandler, i as getQuery, f as executeQuery, h as createError } from '../../_/nitro.mjs';
import { z } from 'zod';
...
const querySchema = z.object({
  ...
  sourceType: z.enum(["group_journal", "manual", "import", "ai", "excel", "all"]).optional().default("all"),
  ...
  try {
    ...
    if (params.sourceType && params.sourceType !== "all") {
      if (params.sourceType === "ai") {
        baseQuery += " AND ic.source_type = 'import' AND ic.import_source = 'ai'";
      } else if (params.sourceType === "excel") {
        baseQuery += " AND ic.source_type = 'import' AND (ic.import_source = 'excel' OR ic.import_source IS NULL)";
      } else {

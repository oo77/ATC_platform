...
const execute_post = defineEventHandler(async (event) => {
  const context = await requireAnyPermission(event, [
  ...
  if (config.skipErrors) {
    rowsToImport = rows.filter((row) => {
      ...
      if (row.certificateStatus === "duplicate" && !config.updateExisting)
        return false;
      return true;
    });
  } else {
    rowsToImport = rows.filter(
  ...
  console.log(
    `[CertificateImport] \u041D\u0430\u0447\u0438\u043D\u0430\u044E \u0438\u043C\u043F\u043E\u0440\u0442 ${rowsToImport.length} \u0441\u0435\u0440\u0442\u0438\u0444\u0438\u043A\u0430\u0442\u043E\u0432...`
  );

...
function mapRowToTestAssignment(row) {
  return {
    ...
    allowed_student_ids: row.allowed_student_ids ? JSON.parse(row.allowed_student_ids) : null,
...
async function createTestAssignment(data, userId) {
  ...
  await executeQuery(
    ...
    [
      ...
      "scheduled",
      userId || null,
      data.allowed_student_ids ? JSON.stringify(data.allowed_student_ids) : null,
      now,
      now
    ]
  );
...
async function updateTestAssignment(id, data) {
  ...
  const updates = [];
  const params = [];
  if (data.time_limit_override !== void 0) {
    updates.push("time_limit_override = ?");
    params.push(data.time_limit_override);
  }
  if (data.passing_score_override !== void 0) {
  ...
  if (data.allowed_student_ids !== void 0) {
...
async function getStudentAssignments(studentId, filters = {}) {
  ...
  queryParams.push(studentId);
  conditions.push(`
    (ta.allowed_student_ids IS NULL OR JSON_CONTAINS(ta.allowed_student_ids, ?))
  `);
  queryParams.push(JSON.stringify(studentId));
  if (status) {
...
export { getTestAssignmentById as a, getStudentAssignments as b, createTestAssignment as c, deleteTestAssignment as d, getTestAssignmentByScheduleEventId as g, testAssignmentExistsForEventAndTemplate as t, updateTestAssignment as u };

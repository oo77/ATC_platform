...
import { defineComponent, computed, ref, watch, mergeProps, unref, withCtx, createTextVNode, toDisplayString, createBlock, openBlock, createVNode, createCommentVNode, withDirectives, isRef, vModelText, Fragment, renderList, vModelSelect, resolveDynamicComponent, useSSRContext } from 'vue';
      ...
      if (unref(loading)) {
        ...
        if (unref(columns).length > 0) {
          ...
          ssrRenderList(unref(columns), (column) => {
            ...
            }, "inline-block w-2 h-2 rounded-full"])}"></span><span class="text-xs">${ssrInterpolate(formatColumnDate(column.scheduleEvent.date))}</span><span class="text-[10px] text-gray-400">${ssrInterpolate(formatTimeRange(
          ...
          if (_push2) {
            ...
            if (unref(selectedEvent)) {
              _push2(`<div${_scopeId}><p class="text-sm text-gray-600 dark:text-gray-400 mb-4"${_scopeId}> Занятие: ${ssrInterpolate(formatColumnDate(unref(selectedEvent).scheduleEvent.date))} ${ssrInterpolate(formatTimeRange(
                ...
                "px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors"
              ])}"${_scopeId}> Все (${ssrInterpolate(unref(selectedEvent).scheduleEvent.academicHours)}) </button></div></div><div class="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4"${_scopeId}><p class="text-sm text-gray-500 dark:text-gray-400"${_scopeId}> Будет отмечено ${ssrInterpolate(unref(rows).length)} слушателей </p></div></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div class="flex justify-end gap-3 pt-4"${_scopeId}>`);
...
//# sourceMappingURL=_slug_-B9HcgwAb.mjs.map

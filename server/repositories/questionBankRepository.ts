...
function mapRowToQuestionBankWithStats(
  row: QuestionBankRow,
): QuestionBankWithStats {
  return {
...
export async function getQuestionBanks(
  ...
  const rows = await executeQuery<QuestionBankRow[]>(dataQuery, [
    ...queryParams,
    limit,
    offset,
  ]);
 ...
 */
export async function getQuestionBankByCode(
  code: string,
 ...
 */
export async function questionBankCodeExists(
  code: string,
  excludeId?: string,
): Promise<boolean> {
 ...
 */
export async function getActiveBanksForSelect(): Promise<
  Array<{ id: string; name: string; code: string; questions_count: number }>
> {
  const rows = await executeQuery<QuestionBankRow[]>(
    `SELECT 
      qb.id, 
...
/**
 * Получить детальную статистику по банку вопросов
 */
export async function getQuestionBankStats(
  bankId: string,
): Promise<QuestionBankStats> {
  // 1. Статистика по сложности
  const difficultyRows = await executeQuery<RowDataPacket[]>(
  ...
  difficultyRows.forEach((row) => {
    if (row.difficulty in difficultyStats) {
      difficultyStats[row.difficulty as keyof typeof difficultyStats] =
        row.count;
    }
  });

  // 2. Статистика по языкам
  const languageRows = await executeQuery<RowDataPacket[]>(
  ...
  languageRows.forEach((row) => {
    // Handle null language as 'ru' (default)
    const lang = row.language || "ru";
    if (lang in languageStats) {
      languageStats[lang as keyof typeof languageStats] += row.count;
    } else if (lang === "ru") {
      languageStats.ru += row.count;
    }
  });

  // 3. Общее количество (можно сложить difficulty, но лучше отдельно или взять сумму)

...
interface TestAssignmentRow extends RowDataPacket {
  ...
  created_at: Date;
  updated_at: Date;
  allowed_student_ids?: any; // JSON column return type
  // Extended fields
...
function mapRowToTestAssignmentWithDetails(
...
export async function getTestAssignments(
  ...
  const rows = await executeQuery<TestAssignmentRow[]>(dataQuery, [
    ...queryParams,
    limit,
    offset,
  ]);
 ...
 */
export async function getTestAssignmentById(
  id: string
 ...
 */
export async function testAssignmentExistsForEvent(
  scheduleEventId: string
): Promise<boolean> {
...
export async function createTestAssignment(
  ...
  await executeQuery(
    ...
    [
      ...
      "scheduled",
      userId || null,
      data.allowed_student_ids
        ? JSON.stringify(data.allowed_student_ids)
        : null,
      now,
      now,
    ]
...
export async function updateTestAssignment(
  ...
  if (data.allowed_student_ids !== undefined) {
...
> {
  ...
  // Фильтрация по allowed_student_ids (если задано ограничение)
  conditions.push(`

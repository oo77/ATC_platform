...
export async function upsertAttendance(
  ...
  const rows = await executeQuery<AttendanceRow[]>(selectSql, [
    data.studentId,
    data.scheduleEventId,
  ]);
...
export async function getAttendanceStats(
  ...
  const rows = await executeQuery<StatsRow[]>(sql, [
    studentId,
    groupId,
    disciplineId,
  ]);
  ...
  return {
    ...
    attendancePercent:
      totalHours > 0
        ? Math.round((attendedHours / totalHours) * 100 * 100) / 100
        : 0,
...
export async function getStudentGrades(
  ...
  const sql = `
    ...
    WHERE g.student_id = ?
      AND se.group_id = ?
      AND se.discipline_id = ?
      AND se.event_type IN ('assessment', 'practice')
    ORDER BY se.start_time
  `;

  const rows = await executeQuery<GradeRow[]>(sql, [
    studentId,
    groupId,
    disciplineId,
  ]);
...
export async function upsertGrade(
  ...
  if (existingGrade && existingGrade.isFromTest && !existingGrade.isModified) {
    ...
    existingGrade &&
    existingGrade.isFromTest &&
    existingGrade.isModified
  ) {
  ...
  const rows = await executeQuery<GradeRow[]>(selectSql, [
    data.studentId,
    data.scheduleEventId,
  ]);
  return mapRowToGrade(rows[0]);
...
export async function getFinalGrades(
  ...
  const rows = await executeQuery<FinalGradeRow[]>(sql, [
    groupId,
    disciplineId,
  ]);
...
export async function getJournalData(
  ...
  const studentsSql = `
    ...
    ORDER BY s.full_name
  `;
  const students = await executeQuery<JournalStudentRow[]>(studentsSql, [
    groupId,
  ]);

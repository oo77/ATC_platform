...
): Promise<PaginatedResult<Instructor & { disciplineCount?: number }>> {
...
export async function instructorEmailExists(
  email: string,
  excludeId?: string,
): Promise<boolean> {
 ...
 */
export async function linkInstructorToUser(
  instructorId: string,
  userId: string,
): Promise<void> {
 ...
 */
export async function getInstructorHoursStats(
  instructorId: string,
): Promise<InstructorHoursStats | null> {
 ...
 * @param additionalHours - Дополнительные академические часы

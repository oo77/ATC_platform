...
function getAuthUserFromEvent(event: H3Event): JwtPayload | null {
  ...
  const contextUser = event.context.user as any;
  if (contextUser) {
    return {
...
export async function requireGroupAccess(
  ...
  if (!hasAccess) {
    console.warn(
      `[Permissions] Access denied: user ${context.userId} cannot access group ${groupId}`,
    );
    throw createError({
      statusCode: 403,
      statusMessage: "Forbidden",
...
export async function requireStudentAccess(
  ...
  if (!hasAccess) {
    console.warn(
      `[Permissions] Access denied: user ${context.userId} cannot access student ${studentId}`,
    );
    throw createError({
      statusCode: 403,
      statusMessage: "Forbidden",
 ...
 */
export function matchApiPattern(
  url: string,
  method: string,
 ...
 */
export function findApiPermissionConfig(
  url: string,
  method: string,
): ApiPermissionConfig | null {

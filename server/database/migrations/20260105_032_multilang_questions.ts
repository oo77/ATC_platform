...
export const description =
  "Добавление многоязычной поддержки: language для вопросов и сессий, allowed_languages для шаблонов";
...
export async function up(connection: PoolConnection): Promise<void> {
  ...
  try {
    ...
    if (e.code === "ER_DUP_FIELDNAME") {
      console.log("  ⚠️ Column questions.language already exists. Skipping.");
    } else {
      throw e;
    }
  }
  ...
  try {
    ...
    if (e.code === "ER_DUP_KEYNAME") {
      console.log(
        "  ⚠️ Index idx_questions_language already exists. Skipping.",
      );
    } else {
      console.warn(
        "  ⚠️ Warning creating index idx_questions_language:",
        e.message,
      );
    }
  }
  ...
  try {
    ...
    if (e.code === "ER_DUP_KEYNAME") {
      console.log(
        "  ⚠️ Index idx_questions_bank_language already exists. Skipping.",
      );
    } else {
      console.warn(
        "  ⚠️ Warning creating index idx_questions_bank_language:",
        e.message,
      );
    }
  }
  ...
  try {
    ...
    if (e.code === "ER_DUP_FIELDNAME") {
      console.log(
        "  ⚠️ Column test_templates.allowed_languages already exists. Skipping.",
      );
    } else {
      throw e;
    }
  ...
  try {
    ...
    if (e.code === "ER_DUP_FIELDNAME") {
      console.log(
        "  ⚠️ Column test_sessions.language already exists. Skipping.",
      );
    } else {
      throw e;
    }
  ...
  try {
    ...
    if (e.code === "ER_DUP_KEYNAME") {
      console.log(
        "  ⚠️ Index idx_test_sessions_language already exists. Skipping.",
      );
    } else {
      console.warn(
        "  ⚠️ Warning creating index idx_test_sessions_language:",
        e.message,
      );
    }
  }

...
/**
 * Миграция: Добавление недостающих полей в certificate_templates (часть 2)
 * Дата: 2026-02-09
 * Описание: Добавляет все недостающие колонки для работы с шаблонами сертификатов
 *           (original_file_url, variables, qr_settings, template_data, layout, background_url)
 */

export const description =
  "Add missing columns to certificate_templates (part 2)";
  ...
  for (const column of columnsToAdd) {
    ...
    if (!existing || existing.length === 0) {
      try {
        ...
        // Игнорируем ошибку "Duplicate column" на случай race condition

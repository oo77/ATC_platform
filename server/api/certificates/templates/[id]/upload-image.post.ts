 ...
 * Загрузить изображение для элемента шаблона сертификата (логотип, печать и т.д.)
 */
...
const UPLOAD_DIR = "./public/uploads/certificate-images";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const fileData = formData.find((item) => item.name === "image");
    if (!fileData) {
      throw createError({
        statusCode: 400,
        message: "Поле 'image' не найдено",
      });
    }
    ...
    const ext =
      contentType === "image/svg+xml" ? "svg" : contentType.split("/")[1];
    ...
    const url = `/uploads/certificate-images/${filename}`;
    ...
    console.log(
      `[POST /api/certificates/templates/${id}/upload-image] Загружено изображение: ${filename}`,
    );
    ...
    console.error(
      "[POST /api/certificates/templates/[id]/upload-image] Error:",
      error,
    );

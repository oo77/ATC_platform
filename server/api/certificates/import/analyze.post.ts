...
import {
  parseCertificateExcel,
  analyzeCertificateImportData,
...
export default defineEventHandler(async (event) => {
  ...
  for (const part of formData) {
    ...
    if (part.name === "config" && part.data) {
      try {
        const rawConfig = JSON.parse(Buffer.from(part.data).toString("utf-8"));
  ...
  if (
    config.validityType === "months" &&
    (!config.validityMonths || config.validityMonths <= 0)
  ) {
    throw createError({
      statusCode: 400,
      statusMessage:
        "При ограниченном сроке действия необходимо указать количество месяцев",
    });
  }
  ...
  try {
    ...
    console.log(
      `[CertificateImport] Найдено ${rows.length} строк, начинаю анализ...`,
    );
    ...
    throw createError({
      statusCode: 500,
      statusMessage:
        error instanceof Error ? error.message : "Ошибка анализа файла",
    });
  }
});
